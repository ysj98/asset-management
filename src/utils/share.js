...
export function getObjectKeyValueByOrganId({ organId, projectName }, cb) {
  ...
  return new Promise((resolve, reject) => {
    apiAssets.getObjectKeyValueByOrganId(req).then(({ data: { code, data, message } }) => {
      if (code === '0') {
        let res = data;
...
export function handleTableTotalRow({ columns, dataSource, totalKeyArr, rowKey }) {
  ...
  columns.forEach((ele, index) => {
    ...
    if (totalKeyArr.includes(keyStr)) {
      ...
      currentPageTotalData[keyStr] = isNaN(data)
        ...
        ? Number(data)
        : valueType.includes(keyStr)
        ? Math.round(Number(data) * 100) / 100
  ...
  dataSource.push(currentPageTotalData, allPageTotalData);
}
