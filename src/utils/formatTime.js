...
*/
  ...
  export function dateToString (value, type = 'yyyy-mm-dd hh:mm:ss') {
    ...
    const map = {
      'yyyy-mm-dd hh:mm:ss': `${value.getFullYear()}-${value.getMonth() + 1}-${value.getDate()} ${value.getHours()}:${value.getMinutes()}:${value.getSeconds()}`.replace(/\b(\w)\b/g, '0$1'),
      'yyyy-mm-dd hh:mm': `${value.getFullYear()}-${value.getMonth() + 1}-${value.getDate()} ${value.getHours()}:${value.getMinutes()}`.replace(/\b(\w)\b/g, '0$1'),
  ...
  export function compareTime (begin, end, type = 'time') {
    if (type === 'time') {
      ...
      return (begin instanceof Date && end instanceof Date) ? (+new Date(dateToString(begin, 'yyyy-mm-dd')) - +new Date(dateToString(end, 'yyyy-mm-dd'))) : ''
    }
  }
