...
export function downloadTemplateBatch(data) {
  return axiosPost(assets.assetRegister.downloadTemplateBatch, data);
}
// 资产登记-批量导入
export function uploadTemplateBatch(data) {
  return axiosPost(assets.assetRegister.uploadTemplateBatch, data);
}
// 资产登记-删除
...
export function getFeeTypeList(data) {
  return new Promise((resolve, reject) => {
    if (configs.platform === 'merchants') {
      axiosPost(assets.assetClassSet.getFeeTypeList2, data).then(
        (value) => {
          let {
            data: { data = [], code = -1, message = '解构失败' },
            ...restArgus
          } = value;
          if (data) {
            data = data.map((ele) => {
    ...
    {
      axiosGet(assets.assetClassSet.getFeeTypeList, data).then(
        (value) => {
          resolve(value);
        },
        (error) => {
