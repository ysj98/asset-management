...
export default {
  ...
  methods: {
    query() {
      ...
      // 资产类型参数改变
      data.projectIdList = Tools.deepClone(data.projectIdList).filter(
        (item) => item !== ""
      )
      data.assetTypeList = Tools.deepClone(data.assetTypeList).filter(
        (item) => item !== ""
      )
      data.approvalStatusList = Tools.deepClone(data.approvalStatusList).filter(
        (item) => item !== ""
      )
      data.registerTypeList = Tools.deepClone(data.registerTypeList).filter(
        (item) => item !== ""
      )
      data.objectTypeList = Tools.deepClone(data.objectTypeList).filter(
        (item) => item !== ""
      )
      if (!data.startDate) {
    ...
    getObjectKeyValueByOrganIdFn() {
      ...
      this.$api.assets.getObjectKeyValueByOrganId(obj).then((res) => {
        if (Number(res.data.code) === 0) {
          ...
          this.projectIdListOpt = [
            ...Tools.deepClone(projectIdListOpt),
            ...result,
          ]
        } else {
          this.$message.error(res.data.message)
    ...
    projectIdListSelect(value) {
      this.$nextTick(function() {
        this.queryCondition.projectIdList = this.handleMultipleSelectValue(
          value,
    ...
    registerTypeListSelect(value) {
      this.$nextTick(function() {
        this.queryCondition.registerTypeList = this.handleMultipleSelectValue(
          value,
          this.queryCondition.registerTypeList,
          this.registerTypeListOpt
        )
      })
    },
    ...
    platformDictFn(code) {
      this.$api.assets.platformDict({ code }).then((res) => {
        if (res.data.code === "0") {
          ...
          if (code === "asset_type") {
            this.assetTypeListOpt = [
              ...Tools.deepClone(assetTypeListOpt),
              ...arr,
            ]
          }
          // 登记类型
          if (code === "AMS_REGISTER_TYPE") {
            this.registerTypeListOpt = [
              ...Tools.deepClone(registerTypeListOpt),
              ...arr,
            ]
          }
        } else {
