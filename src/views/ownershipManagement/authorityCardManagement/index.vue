  ...
  <div class="ownershipRegistration">
    <SearchContainer v-model="toggle" @input="searchContainerFn" :contentStyle="{ paddingTop: '16px' }" style="position: relative">
      <div slot="headerBtns">
        ...
        <SG-Button v-power="ASSET_MANAGEMENT.ASSET_ACM_EXPORT" type="primary" style="margin-right: 8px" @click="exportData"
          ><segiIcon type="#icon-ziyuan10" class="mr10" />导出</SG-Button
        >
        <SG-Button icon="plus" type="primary" v-power="ASSET_MANAGEMENT.ASSET_ACM_NEW" @click="newChangeSheetFn" style="margin-right: 8px"
          >新建权证</SG-Button
        >
        <SG-Button type="primary" v-power="ASSET_MANAGEMENT.ASSET_ACM_NEW" :disabled="control" @click="delBatch">批量注销权证</SG-Button>
        <SG-Button v-power="ASSET_MANAGEMENT.ASSET_ACM_SETTING" icon="setting" @click="changeListSettingsModal(true)" style="margin: 0 10px"
          >列表设置</SG-Button
        >
        <SG-Button v-power="ASSET_MANAGEMENT.zcgl_asset_acm_custom_labels" :disabled="control" @click="tagsFn" style="margin: 0 10px"
          >权证标签</SG-Button
        >
      </div>
      ...
      <div slot="contentForm" class="search-content-box">
        <div class="search-from-box">
          ...
          <a-select :style="allStyle" showSearch :filterOption="filterOption" placeholder="全部权属人" v-model="queryCondition.obligeeId">
            <a-select-option :title="item.name" v-for="(item, index) in obligeeIdData" :key="index" :value="item.value">{{
              item.name
            }}</a-select-option>
          </a-select>
          <a-select
          ...
          >
            <a-select-option :title="item.name" v-for="(item, index) in approvalStatusListData" :key="index" :value="item.value">{{
              item.name
            }}</a-select-option>
          </a-select>
          ...
          <a-select :style="allStyle" showSearch :filterOption="filterOption" placeholder="全部附件状态" v-model="queryCondition.attachmentStatus">
            <a-select-option :title="item.name" v-for="(item, index) in attachmentStatus" :key="index" :value="item.value">{{
              item.name
            }}</a-select-option>
          </a-select>
          <a-select
          ...
          >
            <a-select-option :title="item.name" v-for="(item, index) in warrantTagsList" :key="index" :value="item.value">{{
              item.name
            }}</a-select-option>
          </a-select>
        </div>
...
export default {
  ...
  methods: {
    ...
    downTemplate() {
      ...
      exportDataAsExcel(
        `import_template_qszj.xlsx?organId=${this.queryCondition.organId}`,
        this.$api.tableManage.downloadTemplate,
        '权证导入模板.xlsx',
        this
      );
    },
    ...
    uploadFile(file) {
      ...
      this.$api.tableManage
        .importOwnCardData(query, fileData)
        .then((r) => {
    ...
    exportData() {
      ...
      this.$api.ownership.warrantExport(data).then(
        (res) => {
    ...
    // 权属形式
    platformDict() {
      this.$api.assets.platformDict({ code: 'AMS_OWNER_TYPE' }).then(
        (res) => {
          if (+res.data.code === 0) {
            let data = res.data.data;
            let arr = [
...
.sg-message > .main.confirmDelete > .content,
.sg-message > .main.custom > .content,
.sg-message > .main.error > .content,
.sg-message > .main.info > .content,
.sg-message > .main.loading > .content,
.sg-message > .main.loadingErr > .content,
.sg-message > .main.networkErr > .content,
.sg-message > .main.success > .content {
