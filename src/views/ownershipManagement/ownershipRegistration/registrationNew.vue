        ...
        <a-form :form="form" @submit="handleSubmit">
            ...
            <a-form-item :colon="false" v-bind="formItemLayout">
              ...
              <a-input
                ...
                v-decorator="[
                  'registerName',
                  {
                    rules: [{ required: true, max: 60, whitespace: true, message: '请输入登记单名称(不超过60字符)' }],
                    initialValue: newEditSingleData.registerName,
                  },
                ]"
              />
            </a-form-item>
          ...
          <a-col class="playground-col" :span="8">
            <a-form-item :colon="false" v-bind="formItemLayout">
              ...
              >
                <a-select-option :title="item.name" v-for="item in changeTypeData" :key="item.value" :value="item.value">
                  {{ item.name }}
                </a-select-option>
              </a-select>
            </a-form-item>
...
export default {
  ...
  methods: {
    ...
    change(files, e) {
      ...
      this.$api.ownership.shipImportData(this.formData).then(
        (res) => {
          if (res.data.code === '0') {
            e.target.value = '';
    ...
    platformDictFn() {
      Promise.all([this.$api.assets.platformDict({ code: 'AMS_REGISTER_TYPE' }), this.$api.assets.platformDict({ code: 'asset_type' })]).then(
        (res) => {
          // 登记类型
