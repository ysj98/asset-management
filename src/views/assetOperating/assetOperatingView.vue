...
<template>
  <div class="assetRegister">
    <SG-SearchContainer size="fold" background="white" v-model="toggle" @input="searchContainerFn">
      <div slot="headBtns">
        <SG-Button type="primary" v-power="ASSET_MANAGEMENT.ASSET_OPERATING_VIEW_EXPORT_FILE" @click="exportFn"
        ...
        <div style="position: absolute; top: 20px; right: 76px; display: flex">
          ...
          <a-select
            ...
            placeholder="全部运营项目"
            :tokenSeparators="[',']"
            @select="operatingObjFn"
            v-model="queryCondition.operatingObject"
          >
...
export default {
  ...
  methods: {
    ...
    // 运营项目
    getOperatingObj() {
      this.$api.basics.queryCommunityListByOrganId({ organId: this.queryCondition.organId.split(',')[0] }).then((res) => {
    ...
    pageListStatistics(form) {
      this.$api.assets
        .getOperationScheduleStatistics(form)
        .then((r) => {
          let res = r.data;
