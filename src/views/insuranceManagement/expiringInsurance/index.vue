  ...
  <div>
    ...
    <a-table v-bind="tableObj" size="middle">
      ...
      <template slot="action" slot-scope="text, record">
        <a class="handle_sty" @click="handleClick(record)"> 详情 </a>
      </template>
    </a-table>
  ...
  methods: {
    init(organId, load) {
      ...
      this.$api.assetInsurance
        .queryExpiringInsuranceList(data)
        .then((res) => {
          if (res.data.code === '0') {
    ...
    getCount(data) {
      ...
      this.$api.assetInsurance
        .getExpiringInsuranceCount(data)
        .then((res) => {
          if (res.data.code === '0') {
    ...
    // },
    statusClick(val) {
      this.insuranceStatusList = val.includes('-1') ? [] : val;
    },
    typeClick(val) {
