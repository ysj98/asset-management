<template>
  <div>
    ...
    <search-container size="fold" v-model="fold">
      <div slot="headerBtns">
        <SG-Button v-power="ASSET_MANAGEMENT.INSURANCE_INSURANCE_POLICY_CREATE" type="primary" @click="addPolicy">新建</SG-Button>
      </div>
      <div slot="headerForm">
  ...
  methods: {
    init(organId, load) {
      let {
        ...
        insuranceStatusList,
        insuranceTypeList,
        organProjectBuildingValue: { projectId },
      } = this;
      let data = {
        organId: organId,
    ...
    statusClick(val) {
      this.insuranceStatusList = val.includes('-1') ? '' : val.join(',');
    },
    typeClick(val) {
      this.insuranceTypeList = val.includes('0') ? [] : val;
    },
    companyClick(val) {
      this.insuranceCompanyIdList = val.includes('0') ? [] : val;
    },
    queryTableData() {
      let {
    ...
    handleClic(val, record) {
      ...
      if (val === '2') {
        ...
        this.$api.assetInsurance
          .deleteInsurance({ insuranceId: record.insuranceId })
          .then((res) => {
            console.log(res);
            if (res.data.code === '0') {
      ...
      if (val === '1') {
        ...
        this.$router.push({
          ...
          query: { insuranceId: record.insuranceId, organId: organId, organName: organName },
        });
      }
      if (val === '3') {
      ...
      const list = [
        {
          ...
          tip: '保险公司',
          optionName: 'companyList',
          model: 'insuranceCompanyId',
          obj: { value: '', label: '全部保险公司' },
        },
          ...
          .then((r) => {
            ...
            if (res && String(res.code) === '0') {
              if (res.data.length > 0) {
                ...
                return (this[optionName] = res.data.map(
                  (item) => (
                    m.obj,
                    {
                      label: item.name,
