<template>
  <div class="asset-project-view-detail">
    ...
    <SG-Title title="资产信息" />
    <a-table v-bind="tableObj" size="middle"> </a-table>
  ...
  methods: {
    ...
    getDetailBaseInfo() {
      this.$api.assetInsurance
        .getDetailBaseInfo({ insuranceId: this.insuranceId })
        .then((res) => {
          if (res.data.code === '0') {
    ...
    getDetailAssetInfo() {
      ...
      this.$api.assetInsurance
        .getDetailAssetInfo({ insuranceId: insuranceId, pageNum: pageNo, pageSize: pageLength })
        .then((res) => {
          if (res.data.code === '0') {
