<template>
  <div class="add_policy">
    <a-form :form="form" :layout="'vertical'">
      ...
      <div class="main_info">
        <a-row :gutter="24">
          <a-col :span="8">
            <a-form-item>
              <div slot="label" class="label_tit">保险单号<span>(必填):</span></div>
              <a-input v-decorator="['insuranceCode', { initialValue: form.insuranceCode }]" placeholder="请输入保险单号" />
            </a-form-item>
          </a-col>
          <a-col :span="8">
            <a-form-item label="组织机构名称:" class="label_tit">
              <a-input :disabled="true" v-model="organName" />
            </a-form-item>
          </a-col>
          <!-- <a-col :span="8">
          <a-form-item>
          ...
          <a-col :span="8">
            <a-form-item>
              <div slot="label" class="label_tit">保险类型<span>(必填):</span></div>
              <a-select
        ...
        <a-row :gutter="24">
          <a-col :span="8">
            <a-form-item>
              <div slot="label" class="label_tit">投保人<span>(必填):</span></div>
              <a-input v-decorator="['policyHolder', { initialValue: form.policyHolder }]" placeholder="请输入投保人" />
            </a-form-item>
          </a-col>
          <a-col :span="8">
            <a-form-item>
              <div slot="label" class="label_tit">保险公司<span>(必填):</span></div>
              <a-select
          ...
          <a-col :span="8">
            <a-form-item>
              <div slot="label" class="label_tit">保险公司电话<span>(非必填):</span></div>
              <a-input v-decorator="['insurancePhone', { initialValue: form.insurancePhone }]" placeholder="请输入保险公司电话" />
            </a-form-item>
          </a-col>
        </a-row>
        <a-row :gutter="24">
          <a-col :span="8">
            <a-form-item>
              <div slot="label" class="label_tit">保单金额(合计)<span>(必填):</span></div>
              <a-input @blur="inputClick($event)" v-decorator="['policyAmount', { initialValue: form.policyAmount }]" placeholder="请输入保单金额" />
            </a-form-item>
          </a-col>
          <a-col :span="8">
            <a-form-item>
              <div slot="label" class="label_tit">保险有效期<span>(必填):</span></div>
              <SG-DatePicker
    ...
    <div class="button-box">
      <SG-Button :disabled="selectedRowKeys.length <= 0" class="buytton-nav" style="margin-right: 10px" weaken @click="deleteData">删除</SG-Button>
      <SG-Button class="buytton-nav" type="primary" style="margin-right: 10px" weaken @click="addTheAsset">添加资产</SG-Button>
    </div>
    <a-table v-bind="tableObj" :row-selection="{ selectedRowKeys: selectedRowKeys, onChange: onSelectChange }" size="middle">
...
export default {
  ...
  created() {
    ...
    if (this.$route.query.insuranceId) {
      // 编辑
      this.$api.assetInsurance
        .getDetailBaseInfo({ insuranceId: this.$route.query.insuranceId })
        .then((res) => {
          if (res.data.code === '0') {
  ...
  methods: {
    ...
    getDetailAssetInfo(insuranceId) {
      this.$api.assetInsurance
        .getDetailAssetInfo({ insuranceId, pageNum: 1, pageSize: 100 })
        .then((res) => {
          if (res.data.code === '0') {
    ...
    save() {
      let { insuranceCode, insuranceType, policyHolder, insuranceCompanyId, policyAmount, projectId, rangeValue, insurancePhone } =
        this.form.getFieldsValue();
      if (!insuranceCode || !insuranceType || !policyHolder || !insuranceCompanyId || !policyAmount || !rangeValue) {
        // !projectId
      ...
      this.$api.assetInsurance
        .addOrUpdateAssetInsurance(data)
        .then((res) => {
          // console.log(res)
          let {
    ...
    getDictData() {
      ...
      list.forEach((m) => {
        ...
        this.$api.basics
          .organDict({ code, organId: this.organId })
          .then((r) => {
            let res = r.data;
    ...
    getProjects() {
      this.$api.assets
        .getObjectKeyValueByOrganId({ organId: this.organId })
        .then((res) => {
