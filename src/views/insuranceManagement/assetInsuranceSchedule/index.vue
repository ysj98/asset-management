<template>
  <div>
    ...
    <search-container size="fold" v-model="fold">
      <div slot="headerBtns">
        <SG-Button v-power="ASSET_MANAGEMENT.INSURANCE_INSURANCE_POLICY_EXPORT" type="primary" @click="handleExport">导出</SG-Button>
      </div>
      <div slot="headerForm">
  ...
  methods: {
    init(organId, load) {
      let {
        ...
        assetName,
        provinceCityDistrictValue: { city, district, province },
      ...
      this.$api.assetInsurance
        .findAssetRegInsuranceList(data)
        .then((res) => {
          if (res.data.code === '0') {
    ...
    getCount(data) {
      this.$api.assetInsurance
        .getAssetRegInsuranceTotal(data)
        .then((res) => {
          if (res.data.code === '0') {
