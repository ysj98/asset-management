...
import {queryVirCommunityListByUserIdV2} from '@/api/basics.js'
export default {
  ...
  methods: {
    ...
    query () {
      ...
      queryVirCommunityListByUserIdV2(data).then(res => {
        if (res.data.code === '0') {
          let arr = []
          // 如果可以选择全部项目
          if (this.hasAll) {
          ...
          res.data.data.resultList.forEach(item => {
            let o = {name: item.organName, value: item.organId, parOrganId: item.parOrganId, communityId: item.communityId}
            arr.push(o)
          })
          this.options = arr
          if (this.value) {
