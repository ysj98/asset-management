      ...
      <div class="particulars-obj">
        <a-row class="playground-row">
          ...
          <a-col class="playground-col" :span="8"
            >计划执行日期：{{ particularsData.effDate ? `${particularsData.effDate} - ${particularsData.expDate}` : '--' }}</a-col
          >
    ...
    <div class="particulars-nav">
      ...
      <div class="particulars-obj">
        <a-row class="playground-row">
          ...
          <a-col class="playground-col" :span="6"
            >提前生成任务时间：{{ `${particularsData.preNum}  ${+particularsData.preUnit == 1 ? '天' : '小时'}` || '--' }}</a-col
          >
        </a-row>
        <div class="table-layout-fixed table-border">
          <a-table :loading="loading" :columns="previewColumns" :dataSource="taskExecution" size="middle" :pagination="false"> </a-table>
          <no-data-tips v-show="taskExecution.length === 0"></no-data-tips>
          <SG-FooterPagination :pageLength="task.pageSize" :totalCount="task.totalCount" v-model="task.pageNum" @change="taskChange" />
        </div>
      </div>
    </div>
    <div class="particulars-nav">
      ...
      <div class="particulars-obj">
        <div class="table-layout-fixed table-border">
          <a-table :loading="loading" :columns="executiveColumns" :dataSource="executiveLogging" size="middle" :pagination="false"> </a-table>
