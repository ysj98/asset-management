  ...
  <div class="newPlan">
          ...
          <a-row class="playground-row">
              ...
              <a-form-item v-bind="formItemLayout" label="计划名称：">
                <a-input
                  ...
                  v-decorator="[
                    'planName',
                    {
                      rules: [{ required: true, max: 30, whitespace: true, message: '请输入计划名称(不超过30字符)' }],
                      initialValue: newCardData.planName,
                    },
                  ]"
                />
              </a-form-item>
            ...
            <a-col class="playground-col" :span="8">
              <a-form-item v-bind="formItemLayout" label="所属机构：">
                ...
                >
                  <a-select-option :title="item.name" v-for="item in organIdData" :key="item.value" :value="item.value">
                    {{ item.name }}
                  </a-select-option>
                </a-select>
              </a-form-item>
              ...
              <a-form-item v-bind="formItemLayout" label="计划执行日期：">
                <SG-DatePicker
                  ...
                  v-decorator="[
                    'defaultValue',
                    {
                      rules: [{ type: 'array', required: true, whitespace: true, message: '请计划执行日期' }],
                      initialValue: newCardData.defaultValue,
                    },
                  ]"
                  format="YYYY-MM-DD"
                ></SG-DatePicker>
      ...
      <div class="newPlan-nav">
        ...
        <div class="newPlan-obj">
          ...
          <div class="table-layout-fixed table-border">
            <a-table :loading="loading" :columns="columns" :dataSource="dataSourceReportBill" size="middle" :pagination="false">
              <template v-for="(item, index) in columns" :slot="item.dataIndex" slot-scope="text, record, index">
                ...
                <a-checkbox
                  v-else
                  :disabled="record[item.dataIndex + 'disabled']"
                  :checked="record[item.dataIndex]"
                  @change="checkboxFn(record, index, item.dataIndex)"
                ></a-checkbox>
              </template>
            </a-table>
        ...
        <div class="newPlan-obj">
          ...
          <div class="table-layout-fixed table-border">
            ...
            <div class="table-layout-fixed table-border-lr">
              <a-table :loading="table.loading" :columns="table.previewColumns" :dataSource="table.tableData" size="middle" :pagination="false">
                <!-- 填报人表头 -->
    ...
    <div>
      ...
      <selectStaffOrPost
        ...
        :allOrganId="organIdData"
        :selectType="selectType"
        @change="changeSelectStaffOrPost"
        :selectOptList="selectOptList"
      />
    </div>
    <!-- 选资产 -->
    <projectModal
