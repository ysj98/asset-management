...
<template>
  <div class="countingTask pb70">
    <Cephalosome :rightCol="18" :leftCol="6">
      <div slot="col-l">
        <div class="nav">
          <SG-Button icon="plus" type="primary" style="margin-right: 10px" v-power="ASSET_MANAGEMENT.zcgl_submit_plans_new" @click="goPage('new')"
            >新建呈报计划</SG-Button
          >
      ...
      <div slot="col-r">
        <div class="nav">
          ...
          <a-select
            ...
            placeholder="全部呈报表单"
            :tokenSeparators="[',']"
            @select="reportBillIdFn"
            v-model="queryCondition.reportBillId"
          >
            <a-select-option :title="item.name" v-for="(item, index) in reportBillData" :key="index" :value="item.value">{{
              item.name
            }}</a-select-option>
          </a-select>
          <a-select
        ...
        >
          <template slot="operation" slot-scope="text, record">
            <div class="tab-opt">
              ...
              <span
                @click="goPage('edit', record)"
                v-power="ASSET_MANAGEMENT.zcgl_submit_plans_edit"
                v-show="+record.approvalStatus === 0 || +record.approvalStatus === 3"
                >编辑</span
              >
              <span
                @click="goPage('delete', record)"
                v-power="ASSET_MANAGEMENT.zcgl_submit_plans_delete"
                v-show="+record.approvalStatus === 0 || +record.approvalStatus === 3"
                >删除</span
              >
              <span
                @click="goPage('reverseAudit', record)"
                v-power="ASSET_MANAGEMENT.zcgl_submit_plans_reverse_audit"
...
export default {
  ...
  data() {
    return {
      numList: [
        { title: '全部', key: 'all', value: 0, fontColor: '#324057' },
        { title: '草稿', key: 'draft', value: 0, bgColor: '#4BD288' },
        { title: '待审批', key: 'await', value: 0, bgColor: '#1890FF' },
        { title: '已驳回', key: 'reject', value: 0, bgColor: '#DD81E6' },
        { title: '已审批', key: 'complete', value: 0, bgColor: '#FD7474' },
