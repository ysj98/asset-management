  ...
  <div class="table_edit">
    ...
    <div style="margin-left: 40px">
      <a-row v-if="type == 'edit'" style="line-height: 32px">
        <a-col :span="8">
          <span style="margin-right: 12px; color: #282d5b"> <span style="color: #e4393c">*</span>填报结果: </span>
        ...
        <a-col :span="16">
          <span style="float: left"> <span v-if="result === '0'" style="color: #e4393c">*</span>填报说明: </span>
      ...
      <div style="margin: 20px 0">
        ...
        <SG-Button v-else icon="export" @click="exportTemplate" :loading="exportBtnLoading"> 导出填报模板 </SG-Button>
        <a-upload
          accept=".xls"
      ...
      <a-form :form="form" v-if="customColumns.length">
        <a-table v-bind="tableObj" class="table_box">
          <a-table-column v-for="m in customColumns" :key="m.columnName" :dataIndex="m.columnName" :fixed="m.fixed || false" :width="m.width || 180">
            <!--自定义表头-->
            <span slot="title" @click="handleSameTableCell(m)">
              <span style="color: #e4393c; vertical-align: middle">{{ m.columnNeed ? '*' : '' }}</span>
              <span :style="`${m.columnName === 'cardCode' || m.columnName === 'assetCode' ? '' : 'color: #0084FF; cursor: pointer;'}`">{{
                m.columnDesc
              }}</span>
            </span>
            <!--Table Cell slot-->
            <template slot-scope="text, record">
              ...
              <a-form-item v-if="specialArr.includes(m.columnName)" :key="record.key">
                ...
                />
                <span class="select_mask" @click="handleSelectAsset(record.key)" v-if="['cardName', 'assetName'].includes(m.columnName)"></span>
              </a-form-item>
              <!--columnType === 1 文本输入域-->
              ...
              <a-form-item v-else>
                ...
                <a-input
                  type="hidden"
                  v-decorator="[
                    `${assetType === 'Card' ? 'cardId' : 'assetId'}_${record.key}`,
    ...
    >
      <!--columnType === 1 文本输入域-->
      <a-input v-if="columnType === 1" v-model.trim="sameCellValue" @change="validateValLength($event)" :placeholder="`请输入${columnName}`" />
      <!--columnType === 2 数字输入域-->
      <a-input-number
      ...
      />
      <!--columnType === 3 日期选择域-->
      <a-date-picker allowClear style="width: 100%" v-model="sameCellValue" v-else-if="columnType === 3" :placeholder="`请选择${columnName}`" />
      <!--columnType === 4 下拉选择域-->
      <a-select
        allowClear
        style="width: 100%"
      ...
      />
      <!--columnType === 5 年月日期选择域-->
      <a-month-picker allowClear style="width: 100%" v-model="sameCellValue" v-else-if="columnType === 5" :placeholder="`请选择${columnName}`" />
    </SG-Modal>
...
export default {
  ...
  methods: {
    ...
    queryColumns() {
      const {
        taskInfo: { reportBillId, reportPlanId },
        taskId,
      } = this;
      if (!reportBillId && !reportPlanId) {
        return false;
      }
      // 清空旧数据
      this.dateArr = [];
      ...
      this.$api.reportManage
        .queryReportBillColumn({ reportBillId, reportPlanId })
        .then((r) => {
    ...
    queryTableData(dateArr) {
      this.tableObj.loading = true;
      this.$api.reportManage
        .queryTaskDetailList({ reportTaskId: this.taskId })
        .then((r) => {
          this.tableObj.loading = false;
          let res = r.data;
    ...
    exportTemplate() {
      ...
      this.$api.reportManage
        .exportDetailTemplate({ reportBillId })
        .then((res) => {
    ...
    importBatchData({ file }) {
      const {
        taskInfo: { reportBillId, organId, projectId },
      } = this;
      if (!projectId) {
      ...
      this.$api.reportManage
        .importTaskdetailList(query, fileData)
        .then((r) => {
          this.tableObj.loading = false;
          let res = r.data;
          if (res && String(res.code) === '0') {
