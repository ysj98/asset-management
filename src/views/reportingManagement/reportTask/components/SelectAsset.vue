  ...
  >
    <div class="select_asset">
      <a-row :gutter="8" style="margin-bottom: 10px">
        ...
        <a-col :span="7">
          <a-input-search v-model.trim="name" style="width: 100%" @search="queryData()" @pressEnter="queryData()" placeholder="请输入名称" />
        </a-col>
      </a-row>
      <a-table
        size="small"
      ...
      <SG-FooterPagination noPageTools v-bind="paginationObj" @change="({ pageNo, pageLength }) => queryData({ pageNo, pageLength })" />
    </div>
  </SG-Modal>
</template>
  ...
  methods: {
    ...
    queryCardList({ pageLength = 10, pageNo = 1 }) {
      ...
      let form = {
        ...
        cardName: name,
        assetType,
        assetCategoryId: objectType,
        pageSize: pageLength,
        pageNum: pageNo,
      };
      this.$api.assets
        .queryCardPageList(form)
        .then((r) => {
          let res = r.data;
          if (res && res.code.toString() === '0') {
            this.loading = false;
