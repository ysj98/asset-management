...
export default {
  ...
  methods: {
    ...
    createIconMarker(result = []) {
      ...
      result.forEach((item) => {
        ...
        if (longitude && latitude) {
          let myCompOverlay = new ComplexCustomOverlay(new BMap.Point(longitude, latitude), item);
    ...
    createBaiduMap() {
      ...
      // 复杂的自定义覆盖物
      function ComplexCustomOverlay(point, opt = [{ resourceType: '1', resourceName: '' }]) {
        // 是否 有重复 点位 的资源
      ...
      ComplexCustomOverlay.prototype.initialize = function (map) {
        ...
        tipDiv.appendChild(document.createTextNode(tipStr));
        // 创建箭头
