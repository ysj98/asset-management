...
<template>
  <div class="rentGainsRegister">
    <SG-SearchContainer size="fold" background="white" v-model="toggle" @input="searchContainerFn">
      <div slot="headBtns">
        <SG-Button class="fl" style="margin-right: 20px" type="primary" @click="registerFn" v-power="ASSET_MANAGEMENT.INVEST_INCOME_NEW"
          >收益登记</SG-Button
        >
        <SG-Button icon="export" :loading="exportBtnLoading" @click="exportFn" v-power="ASSET_MANAGEMENT.INVEST_EARNINGS_REGISTER">导出</SG-Button>
      ...
      this.$api.assetRent.getIncomePageList(obj).then((res) => {
        if (Number(res.data.code) === 0) {
          ...
          data.forEach((item, index) => {
            ...
            item.status === 1 ? (item.status = '有效') : (item.status = '已作废');
          });
          this.tableData = data;
