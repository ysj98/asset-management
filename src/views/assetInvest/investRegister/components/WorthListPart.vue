  ...
  <div class="worth_list">
      ...
      <div style="margin-bottom: 8px; text-align: right">
        <div class="box">
          <div class="left" style="height: 100%">
            资产数量：{{ tableObj.dataSource.length }}
    ...
    <div v-if="type == 'detail'">
      ...
      <div style="margin-left: 40px">
        <div style="margin-bottom: 8px; text-align: right">
          <div class="box">
            <div class="left" style="height: 100%">总收益：{{ profitSum }}</div>
            <div class="right" style="margin-bottom: 8px"></div>
          </div>

          <a-table v-bind="profitTableObj" class="custom-tables" bordered>
            <template slot="assetCode" slot-scope="text">
              <tooltip-text width="150" :text="text" />
            </template>
            <template slot="assetName" slot-scope="text">
  ...
  data() {
    return {
        ...
        columns:
          this.type == 'add' || this.type == 'edit'
            ? [
                ...
                { title: '操作', dataIndex: 'opt', scopedSlots: { customRender: 'opt' } },
              ]
            : [
                { title: '序号', dataIndex: 'index' },
                { title: '资产编码', dataIndex: 'assetCode' },
      ...
      profitTableObj: {
        ...
        columns: [
          { title: '序号', dataIndex: 'index' },
          { title: '收入编号', dataIndex: 'incomeId' },
          { title: '费用科目', dataIndex: 'feeSubjectName' },
          { title: '账期', dataIndex: 'accountingPeriod' },
          { title: '收益金额(元)', dataIndex: 'amount' },
          { title: '备注', dataIndex: 'remark', scopedSlots: { customRender: 'remark' } },
        ],
      },
  ...
  methods: {
    ...
    queryAssetListByRegisterId({ pageNo = 1, pageLength = 10, type }) {
      ...
      this.$api.assetInvest
        .getInvestDetailPageList({ investOrderId: registerId, pageNum: pageNo, pageSize: pageLength })
        .then((r) => {
          ...
          if (this.type == 'detail') {
            this.$api.assetRent
              .getIncomeDetailPageList({ pageNum: pageNo, pageSize: pageLength, orderId: registerId, orderType: 2, status: 1 })
              .then((result) => {
