...
export default {
  ...
  methods: {
          ...
          let form =
            type === 'edit' || type === 'add'
              ? {
                  ...data,
                  investDetail,
                  investOrderId: registerId ? registerId : '',
                  investArea: investAreaTotal,
                  ...dynamicData,
                  approvalStatus: saveWays,
                }
              : { ...data, detailList };
          delete form.assetArea;
          this.$api.assetInvest
            .saveUpdateInvestOrder(form)
            .then((r) => {
              this.spinning = false;
              let res = r.data;
    ...
    queryDetailById(registerId) {
      ...
      this.$api.assetInvest
        .getInvestOrder({ investOrderId: registerId })
        .then((r) => {
          let res = r.data;
    ...
    handleApprove(sign) {
      ...
      this.$api.assetInvest
        .updateInvestOrderStatus({ approvalStatus, investOrderId: registerId })
        .then((r) => {
          this.spinning = false;
          let res = r.data;
          if (res && String(res.code) === '0') {
