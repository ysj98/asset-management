...
<template>
  <div class="assetRegister">
    <SG-SearchContainer size="fold" background="white" v-model="toggle" @input="searchContainerFn">
      <div slot="headBtns">
        <SG-Button type="primary" v-power="ASSET_MANAGEMENT.INVEST_FORM_NEW" @click="handleBtnAction({ type: 'add' })" v-if="organName.length > 0"
          >新建投资单</SG-Button
        >
        <div class="box" style="margin-left: 16px">
          <SG-Button type="primary" v-power="ASSET_MANAGEMENT.ASSET_INVEST_REGISTER" @click="exportFn"
        ...
        <div style="position: absolute; top: 20px; right: 76px; display: flex">
          ...
          <a-input-search
            v-model="queryCondition.investNameOrIdOrContractCode"
            placeholder="投资单名称/投资单编号/合同编号"
            :maxLength="30"
      ...
      <div slot="form" class="formCon">
        <a-select
          ...
          placeholder="全部合同状态"
          :tokenSeparators="[',']"
          @select="investStatusFn"
          v-model="queryCondition.investStatusList"
          :getPopupContainer="
            (triggerNode) => {
              return triggerNode.parentNode || document.body;
            }
          "
        >
          <a-select-option :title="item.name" v-for="(item, index) in investStatusData" :key="index" :value="item.value">{{
            item.name
          }}</a-select-option>
        </a-select>
        <div class="box">
  ...
  methods: {
    ...
    operationFun(type, record) {
      ...
      if (['edit'].includes(type)) {
        ...
        this.$router.push({
          name: '投资登记详情',
          params: { registerId: record.investOrderId, type: 'detail', organId: record.organId, organName: record.organName, queryType: 1 },
        });
      } else if (['approval'].includes(type)) {
        this.$router.push({
          name: '投资登记审批',
          params: { registerId: record.investOrderId, type: 'approval', organId: record.organId, organName: record.organName, queryType: 1 },
        });
        ...
        this.$confirm({
          ...
          onOk() {
            that.loading = true;
            that.$api.assetInvest
              .updateInvestOrderStatus({ investOrderId: record.investOrderId, approvalStatus: 0 })
              .then((r) => {
                that.loading = false;
                let res = r.data;
        ...
        this.$confirm({
          ...
          onOk() {
            that.loading = true;
            that.$api.assetInvest
              .updateInvestOrderStatus({ investOrderId: record.investOrderId, approvalStatus: 4 })
              .then((r) => {
                that.loading = false;
                let res = r.data;
                if (res && String(res.code) === '0') {
    ...
    confirmDelete(registerId) {
      this.loading = true;
      this.$api.assetInvest
        .updateInvestOrderStatus({ investOrderId: registerId, approvalStatus: 4 })
        .then((r) => {
          this.loading = false;
          let res = r.data;
          if (res && String(res.code) === '0') {
