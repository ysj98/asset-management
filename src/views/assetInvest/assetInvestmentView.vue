...
<template>
  <div class="investment_view">
    ...
    <search-container size="fold" background="white" v-model="toggle">
      <div slot="headerBtns">
        <SG-Button icon="import" type="primary" :loading="exportBtnLoading" @click="handleExport" v-power="ASSET_MANAGEMENT.ASSET_INVEST_VIEW_EXPORT"
          >导出</SG-Button
        >
      </div>
      <div slot="headerForm" style="margin-right: 32px; text-align: left">
        <a-row :gutter="8">
          ...
          <a-col :span="4">
            <a-select v-model="queryObj.assetTypeList" :options="$addTitle(assetTypeOptions)" v-bind="selectProperty" placeholder="请选择资产类型" />
          </a-col>
          <a-col :span="4">
            <a-select
              v-model="queryObj.investStatusList"
              :options="$addTitle(investStatusOptions)"
              v-bind="selectProperty"
              placeholder="请选择投资状态"
            />
          </a-col>
          <a-col :span="3">
      ...
      <div slot="contentForm" style="margin-top: 18px">
        <a-row :gutter="8">
          ...
          <a-col :span="4">
            <a-select
              v-model="queryObj.approvalStatusList"
              :options="$addTitle(approveStatusOptions)"
              v-bind="selectProperty"
              placeholder="请选择审批状态"
            />
          </a-col>
...
export default {
  ...
  methods: {
    ...
    queryCategoryOptions() {
      ...
      const {
        organProjectValue: { organId },
        queryObj: { assetTypeList },
      } = this;
      if (!organId || !assetTypeList.length || assetTypeList.includes('-1') || assetTypeList.length > 1) {
        return false;
      }
    ...
    queryTableData({ pageNum = 1, pageSize = 10, actionType }) {
      ...
      let temp = {};
      arr.forEach((k) => (temp[k] = queryObj[k].filter((n) => n !== '-1')));
      ...
      this.$api.assetInvest
        ...
        .catch((err) => this.$message.error(err || '查询列表数据错误'))
    ...
    queryTotalInfo(obj) {
      if (this.tableData.length) {
        this.loading = true;
        this.$api.assetInvest
          .queryInvestViewTotal(obj)
          .then(
            ({
              data: {
                data: { totalInvestArea },
                code,
                msg,
              },
            }) => {
          ...
          .catch((err) => this.$message.error(err || '查询列表汇总数据错误'))
          .finally(() => (this.loading = false));
      }
    },
    ...
    handleExport() {
      if (!this.tableData.length) {
        return this.$message.info('暂无导出数据');
      }
      this.exportBtnLoading = true;
      exportDataAsExcel(this.queryTableData({ actionType: 'export' }), this.$api.assetInvest.exportInvestView, '资产投资一览表.xlsx', this).then(
