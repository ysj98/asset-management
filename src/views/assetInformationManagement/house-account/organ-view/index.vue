  ...
  export default {
    ...
    methods: {
      ...
      queryTableData({pageNo = 1, pageLength = 10, type}) {
        const {sumObj, organId, current} = this
        if (!organId) {
          return this.$message.info('请选择组织机构')
        }
        this.tableObj.loading = true
        this.$api.assets.queryOrganViewList({
          organId, pageSize: pageLength, pageNum: pageNo, flag: current ? (current - 1) : ''
        }).then(({data: res}) => {
          ...
          if (res && String(res.code) === '0') {
            ...
            this.tableObj.dataSource = data.length ? data.concat({...pageSum, organName: '本页合计', organId: '-999'}) : []
            // 查询楼栋面积统计数据
            if (type !== 'search') {
              data.length && this.tableObj.dataSource.push({...sumObj, organName: '全部合计', organId: Date.now()})
            } else {
              this.queryAreaInfo()
            }
      ...
      handleExport() {
        ...
        this.$api.assets.exportOrganView({
          organId, flag: current ? (current - 1) : '', pageSize: 1, pageNum: 1
        }).then(res => {
