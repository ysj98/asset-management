...
export default {
  ...
  methods: {
    ...
    queryTableData({ pageNo = 1, pageLength = 10, type }) {
      ...
      // 查询楼栋面积统计数据
      if (type === 'search' || this.$route.query.organIds) {
        this.queryAssetAreaInfo(form);
      }
    },
    // 合计汇总合并
    totalFn(form) {
    ...
    handleExport(type) {
      ...
      if (type === 'exportAssetBtn') {
        ...
        this.$api.assets.exportAssetViewExcelExam(form).then((res1) => {
          if (+res1.data.code === -1) {
            this[type] = false;
            return this.$message.error(res1.data.message);
          }
          if (+res1.data.code === 0) {
