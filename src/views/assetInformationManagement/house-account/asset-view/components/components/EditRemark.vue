  ...
  <div class="edit-remark">
    <div class="left-box">
      ...
      <div class="content">
        ...
        <span>{{ item.assetName }} </span><span style="float: right" @click="deleteItem(item.assetHouseId)"><SG-Icon type="icon-ic_guanbi" size="12px"></SG-Icon></span></div>
      </div>
    </div>
...
methods: {
  // 查询详情
      queryDetailInfo (assetHouseId) {
        this.spinning = true
        this.$api.assets.queryAssetViewBaseInfo({assetHouseId}).then(r => {
          console.log(r)
          this.spinning = false
          let res = r.data
  ...
  deleteItem (assetHouseId) {
    if (this.asset.length < 2) {return}
    this.asset = this.asset.filter((item) => {return item.assetHouseId !== assetHouseId})
    let temp = []
    this.asset.map(e => {
