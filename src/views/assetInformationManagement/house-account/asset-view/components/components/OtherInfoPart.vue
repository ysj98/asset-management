    ...
    <a-tabs defaultActiveKey="receive" v-model="tabKey" class="title_div">
      <a-tab-pane v-for="(item, itemKey) in infoKeys" :tab="item.title" :key="itemKey">
        ...
        <a-table class="custom-table td-pd10 table-border" :columns="item['table']['columns']" :dataSource="tableData" :pagination="false">
          <span slot="warrantNbr" style="color: #0084ff; cursor: pointer" slot-scope="text, record" @click="openWarrant(record)">{{
            record.warrantNbr
          }}</span>
          <span slot="registerId" style="color: #0084ff; cursor: pointer" slot-scope="text, record" @click="goRegister(record)">{{
            record.registerId
          }}</span>
  ...
  data() {
    return {
      ...
      apiObj: {
        ...
        patrolRecord: {
          api: 'queryAssetViewPatrolDetail',
          tip: '巡查记录',
          param: 'assetId',
          data: 'assetId',
          pagination: true,
          prower: 'power_patrolRecord',
        }, // 巡查记录
        ...
        relatedExpenses: {
          api: 'assetExpenseInfo',
          tip: '相关费用',
          param: 'assetId',
          data: 'assetId',
          pagination: true,
          prower: 'power_relatedExpenses',
        }, // 相关费用
        archive: {
          api: 'queryAssetViewArchiveDetail',
          tip: '档案文件',
          param: 'assetId',
          data: 'assetId',
          pagination: true,
          prower: 'power_archive',
        }, // 档案文件
      ...
      this.$api.assets[api](queryData)
        .then((r) => {
          ...
          if (res && String(res.code) === '0') {
            ...
            obj =
              type === 'archive' || type === 'relatedExpenses' || type === 'patrolRecord'
                ? { detailData, tableData, pagination: { ...this.pagination } }
                : obj;
