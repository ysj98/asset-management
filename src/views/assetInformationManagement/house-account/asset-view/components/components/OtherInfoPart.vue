...
<template>
  <div class="other_info" v-if="Object.keys(infoKeys).length && isPageShow">
    ...
    <a-tabs defaultActiveKey="receive" v-model="tabKey" class="title_div">
      ...
      <a-tab-pane tab="运营信息" key="operationInformation" v-power="ASSET_MANAGEMENT.power_operationInformation">
  ...
  export default {
    ...
    created () {
      for (const key in this.apiObj) {
        ...
        } else if (this.$power.has(ASSET_MANAGEMENT.power_operationInformation)) {
          this.isPageShow = true
        }
      }
    },
