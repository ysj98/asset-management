  ...
  <div>
    <a-row>
      <a-col :span="8">
        ...
        <span style="color: #49505e">{{ transferOperationArea ? transferOperationArea + '㎡' : '' }}</span>
      </a-col>
      <a-col :span="8">
    ...
    <a-table class="custom-table operation" :pagination="false" :columns="table.columns" :dataSource="table.dataSource">
      <span slot="contractCode" style="color: #0084ff; cursor: pointer" slot-scope="text, record" @click="openContract(record.contractId)">{{
        record.contractCode
      }}</span>
    </a-table>
    ...
    <SG-Title title="运营收入信息：">
      ...
      <SG-Button style="min-width: auto" type="primary" @click="queryTable2">查询</SG-Button>
    </SG-Title>
    <a-table class="custom-table operation" :pagination="false" :columns="table2.columns" :dataSource="table2.dataSource"> </a-table>
    <SG-FooterPagination :pageLength="table2.pageSize" :totalCount="table2.totalCount" v-model="table2.pageNum" @change="handleChange2" />
  </div>
</template>
<script>
