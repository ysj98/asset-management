...
export default {
  ...
  methods: {
    ...
    queryHouseAreaInfo(args) {
      ...
      return this.$api.assets[api](param)
        .then((r) => {
          let res = r.data;
    ...
    queryFloorInfo() {
      ...
      this.spinning = true;
      this.$api.assets
        .queryBuildingViewFloorInfo({ buildId, organId, unitId })
        .then((r) => {
          this.spinning = false;
          let res = r.data;
          if (res && String(res.code) === '0') {
    ...
    queryUnit() {
      this.spinning = true;
      this.$api.assets
        .queryBuildingViewUnitByHouseId({ buildId: this.buildId, organId: this.organId })
        .then((r) => {
          this.spinning = false;
          let res = r.data;
          if (res && String(res.code) === '0') {
