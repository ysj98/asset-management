      ...
      <div class="particulars-obj">
        ...
        <keep-alive>
          <necessaryCaaessories
            v-if="this.selectTabValue === '1'"
            ref="necessaryCaaessories"
            :organId="organId"
        ...
        <keep-alive>
          <correlativeCharges
            v-if="this.selectTabValue === '4'"
            ref="correlativeCharges"
            :organId="organId"
            :registerOrderId="registerOrderId"
            :assetType="assetType"
          ></correlativeCharges>
    ...
    query() {
      ...
      this.$api.assets.getRegisterOrderById(obj).then((res) => {
        if (Number(res.data.code) === 0) {
          ...
          this.filepaths = data.otherAttachment.map((ele) => ({
            url: ele.attachmentPath,
            name: ele.oldAttachmentName,
            attachmentId: ele.attachmentId,
          }));
          this.imgFiles = data.imageAttachment.map((ele) => ({
            url: ele.attachmentPath,
            name: ele.oldAttachmentName,
            attachmentId: ele.attachmentId,
          }));
