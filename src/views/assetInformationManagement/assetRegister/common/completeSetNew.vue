              ...
              <a-col :span="12" :class="{ 'h-65': type === 'new' }" v-if="type === 'new'">
                <a-form-item label="名称" v-bind="formItemLayout">
                  <a-select
                    ...
                    @search="handleSearch"
                    :filterOption="false"
                    v-decorator="['assetId', { rules: [{ required: true, message: '请选择类型' }], initialValue: subData.assetId }]"
                    notFoundContent="没有查询到数据"
                  />
                </a-form-item>
              </a-col>
              <a-col v-if="type === 'edit'" :class="{ 'h-40': type === 'edit' }" :span="12"
                ><a-form-item label="资产名称：" v-bind="formItemLayout">{{ examine.assetName || '--' }}</a-form-item></a-col
              >
              <a-col :span="12" :class="{ 'h-65': type === 'new', 'h-40': type === 'edit' }"
                ><a-form-item label="资产编码：" v-bind="formItemLayout">{{ examine.assetCode || '--' }}</a-form-item></a-col
              >
              <a-col :span="12" :class="{ 'h-65': type === 'new', 'h-40': type === 'edit' }"
                ><a-form-item label="资产分类：" v-bind="formItemLayout">{{ examine.objectTypeName || '--' }}</a-form-item></a-col
              >
              <a-col :span="12" :class="{ 'h-65': type === 'new', 'h-40': type === 'edit' }"
                ><a-form-item label="资产位置：" v-bind="formItemLayout">{{ examine.pasitionString || '--' }}</a-form-item></a-col
              >
            </a-form>
          </a-row>
        </div>
        ...
        <div class="mt30 mb30">
          <a-row class="playground-row">
            <a-form :form="form" @submit="handleSubmit">
              ...
              <a-col :span="12" class="h-65">
                <a-form-item label="类型" v-bind="formItemLayout">
                  <a-select
                    ...
                    v-decorator="['matchingType', { rules: [{ required: true, message: '请选择类型' }], initialValue: subData.matchingType }]"
                    notFoundContent="没有查询到数据"
                  />
                </a-form-item>
              </a-col>
              ...
              <a-col :span="12" class="h-65">
                <a-form-item label="价值(元)" v-bind="formItemLayout">
                  <a-input-number
                    ...
                    :min="0"
                    :precision="2"
                    v-decorator="['value', { rules: [{ required: true, message: '请输入价值(支持两位小数)' }], initialValue: subData.value }]"
                  />
                </a-form-item>
              </a-col>
              <a-col :span="12" class="h-65">
                <a-form-item label="数量" v-bind="formItemLayout">
                  <a-input-number
                    ...
                    :max="999999999.99"
                    :min="0"
                    v-decorator="['number', { rules: [{ required: true, message: '请输入数量(正整数)' }], initialValue: subData.number }]"
                  />
                </a-form-item>
              </a-col>
              <a-col :span="12" class="h-65">
                <a-form-item label="计量单位" v-bind="formItemLayout">
                  <a-select
                    ...
                    :allowClear="true"
                    :filterOption="filterOption"
                    v-decorator="['unitOfMeasurement', { rules: [{ message: '请选择计量单位' }], initialValue: subData.unitOfMeasurement }]"
                    notFoundContent="没有查询到数据"
                  />
                </a-form-item>
              </a-col>
              <a-col :span="12" class="h-65">
                <a-form-item>
                  <a-checkbox :checked="subData.isBefore" @change="handleChange"> 接管前附属配套 </a-checkbox>
                </a-form-item>
              </a-col>
              <a-col :span="24">
              ...
              <a-col :span="24">
                <a-form-item :colon="false" v-bind="formItemTextarea">
                  ...
                  <UploadFile v-model="subData.files" type="all" />
                </a-form-item>
              </a-col>
            </a-form>
          </a-row>
