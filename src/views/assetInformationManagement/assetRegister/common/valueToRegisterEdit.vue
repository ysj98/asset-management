...
<template>
  <div class="basicDownload">
    <SG-Modal :centered="true" width="800px" v-model="modalShow" okText="提交" title="编辑价值信息" @cancel="handleCancel" @ok="handleSubmit">
              ...
              <a-col :span="12" class="h-65">
                <a-form-item label="使用期限(月)" v-bind="formItemLayout">
                  <a-input-number
                    ...
                    :precision="0"
                    :max="999"
                    v-decorator="['validPeriod', { rules: [{ required: false, message: '使用期限' }], initialValue: subData.validPeriod }]"
                  />
                </a-form-item>
              </a-col>
              <a-col :span="12" class="h-65">
                <a-form-item label="开始使用日期" v-bind="formItemLayout">
                  <a-date-picker
                    :style="allWidth"
                    placeholder="开始使用日期"
                    v-decorator="['startDate', { rules: [{ required: false, message: '开始使用日期' }] }]"
                  />
                </a-form-item>
              </a-col>
              <a-col :span="12" class="h-65">
                <a-form-item label="已使用期限" v-bind="formItemLayout">
                  <a-input-number
                    ...
                    :max="999"
                    :precision="0"
                    v-decorator="['usedDate', { rules: [{ required: false, message: '已使用期限' }], initialValue: subData.usedDate }]"
                  />
                </a-form-item>
              </a-col>
              <a-col :span="12" class="h-65">
