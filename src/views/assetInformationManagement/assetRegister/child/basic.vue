    ...
    <div class="table-borders" :class="{ overflowX: tableData.length === 0 }">
      <a-table class="table-boxs" :scroll="{ y: 450, x: 2300 }" :columns="columns" :dataSource="tableData" :pagination="false" :loading="loading">
        ...
        <template slot="houseNumber" slot-scope="text, record">
          <a-input v-if="setType === 'new'" placeholder="请输入公安门牌号" v-model="record.houseNumber" />
          <span v-else>{{ text }}</span>
        </template>
      </a-table>
...
export default {
  ...
  methods: {
    ...
    change(files, e) {
      ...
      this.$api.assets.readExcelModel(this.formData).then(
        (res) => {
          if (res.data.code === '0') {
            e.target.value = '';
            let resData = [],
            ...
            this.tableData = this.handleTableDataSourceModeName(resData).map((ele) => ({
              ...ele,
    ...
    handleValidateExcelData(arrData) {
      ...
      let publicData = [];
      if (
        Object.keys(this.ASSET_TYPE_CODE)
          .map((e) => this.ASSET_TYPE_CODE[e])
