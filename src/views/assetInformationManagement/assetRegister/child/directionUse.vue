...
<template>
  <div class="directionUse">
    ...
    <!-- 新增编辑 -->
    <directionUseEdit :is-equipment="isEquipment" @cancel="cancel" v-if="modalShow" ref="directionUseEdit" @allQuery="allQuery"> </directionUseEdit>
...
export default {
  ...
  methods: {
    ...
    change(files, e) {
      ...
      this.$api.assets.usrForImport(this.formData).then(
        (res) => {
          if (res.data.code === '0') {
            e.target.value = '';
