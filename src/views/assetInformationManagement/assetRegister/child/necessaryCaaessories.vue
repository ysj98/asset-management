...
<template>
  <div class="necessaryCaaessories">
    ...
    <div class="table-borders" :class="{ overflowX: tableData.length === 0 }">
      <a-table class="table-boxs" :columns="columns" :loading="loading" :scroll="{ y: 450, x: 2500 }" :dataSource="tableData" :pagination="false">
  ...
  methods: {
    ...
    newlyFn(str, record) {
      ...
      if (str === 'new') {
        ...
        (obj = record), (obj.registerOrderId = this.queryCondition.registerOrderId); //  资产登记ID
    ...
    uploadModeFile(file) {
      ...
      this.$api.assets.batchImportByRgId(fileData).then(
        (res) => {
          if (res.data.code === '0') {
            this.DE_Loding(loadingName).then(() => {
