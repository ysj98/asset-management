...
<template>
  <div class="valueToRegister">
    <div class="button-box">
      <div class="buytton-l">
        <span>资产总原值：{{ statistics.originalValue || '0' }}元</span>
        <span class="p120">累计折旧总金额：{{ statistics.depreciationAmount || '0' }}元</span>
      </div>
...
export default {
  ...
  methods: {
    ...
    change(files, e) {
      ...
      this.$api.assets.valueImport(this.formData).then(
        (res) => {
          if (res.data.code === '0') {
            e.target.value = '';
