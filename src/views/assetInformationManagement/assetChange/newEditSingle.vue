    ...
    <div class="newEditSingle-nav">
        ...
        <a-form :form="form">
            ...
            <a-form-item :colon="false" v-bind="formItemLayout">
              ...
              <a-input
                ...
                v-decorator="[
                  'title',
                  {
                    rules: [{ required: true, max: 50, whitespace: true, message: '请输入变更单名称(不超过50字符)' }],
                    initialValue: newEditSingleData.title,
                  },
                ]"
              />
            </a-form-item>
          ...
          <a-col class="playground-col" :span="8">
            <a-form-item :colon="false" v-bind="formItemLayout">
              ...
              >
                <a-select-option v-for="item in organIdData" :key="item.value" :value="item.value" :title="item.name">{{
                  item.name
                }}</a-select-option>
              </a-select>
            </a-form-item>
          ...
          <a-col class="playground-col" :span="8">
            <a-form-item :colon="false" v-bind="formItemLayout">
              ...
              >
                <a-select-option v-for="item in projectIdData" :key="item.value" :value="item.value" :title="item.name">{{
                  item.name
                }}</a-select-option>
              </a-select>
            </a-form-item>
          ...
          <a-col class="playground-col" :span="8">
            <a-form-item :colon="false" v-bind="formItemLayout">
              ...
              >
                <a-select-option v-for="item in assetTypeData" :key="item.value" :value="item.value" :title="item.name">{{
                  item.name
                }}</a-select-option>
              </a-select>
            </a-form-item>
          ...
          <a-col class="playground-col" :span="8">
            <a-form-item :colon="false" v-bind="formItemLayout">
              ...
              >
                <a-select-option v-for="item in changeTypeDataCom" :key="item.value" :value="item.value" :title="item.name">{{
                  item.name
                }}</a-select-option>
              </a-select>
            </a-form-item>
          ...
          <a-col class="playground-col" :span="8">
            <a-form-item :colon="false" v-bind="formItemLayout">
              ...
              <a-date-picker
                :style="allWidth"
                placeholder="请选择变更日期"
                v-decorator="['changeDate', { rules: [{ required: true, message: '请选择变更日期' }] }]"
              />
            </a-form-item>
          </a-col>
          <!-- 选择变更类型为，资产原值 -->
          <template v-if="changeType === '3'">
            <a-col class="playground-col" :span="8">
              <a-form-item :colon="false" v-bind="formItemLayout">
                ...
                >
                  <a-select-option v-for="item in originalObjectTypeDataCom" :key="item.value" :value="item.value" :title="item.name">{{
                    item.name
                  }}</a-select-option>
                </a-select>
              </a-form-item>
              ...
              <a-col class="playground-col" :span="8">
                <a-form-item :colon="false" v-bind="formItemLayout">
                  ...
                  <a-select
                    ...
                    notFoundContent="没有查询到数据"
                  >
                    <a-select-option v-for="item in originalObjectIdData" :key="item.value" :value="item.value" :title="item.name">{{
                      item.name
                    }}</a-select-option>
                  </a-select>
                </a-form-item>
              ...
              <a-col class="playground-col" :span="8">
                <a-form-item :colon="false" v-bind="formItemLayout">
                  ...
                  <a-select
                    ...
                    notFoundContent="没有查询到数据"
                  >
                    <a-select-option v-for="item in shareWayData" :key="item.value" :value="item.value" :title="item.name">{{
                      item.name
                    }}</a-select-option>
                  </a-select>
                </a-form-item>
      ...
      <div class="tab-nav">
        ...
        <div class="button-box">
          <SG-Button v-show="changeType === '4'" class="buytton-nav" weaken @click="addTheAsset('batchUpdate')">批量更新</SG-Button>
          <SG-Button
            v-show="changeType === '4'"
            class="buytton-nav"
            type="primary"
        ...
        <div class="table-layout-fixed" v-if="columnsCom.length !== 0" :class="{ 'table-border': tableData.length != 0 }">
          <a-table :scroll="scroll" :columns="columnsCom" :dataSource="tableData" size="middle" :pagination="false">
            ...
            <template #assetArea="text, record">
              {{
                [$store.state.ASSET_TYPE_STRING.EQUIPMENT, $store.state.ASSET_TYPE_CODE.EQUIPMENT].includes(String(record.assetType))
                  ? '/'
                  : record.assetArea
              }}
            </template>
            ...
            <template v-if="changeType === '1'" slot="transferOperationArea" slot-scope="text, record">
              <a-input-number size="small" :min="0.01" :step="1.0" :precision="4" v-model="record.transferOperationArea" />
            </template>
            <!-- 交付物业 -->
            <template v-if="changeType === '2'" slot="transferArea" slot-scope="text, record">
              <a-input-number size="small" :min="0.01" :step="1.0" :precision="4" v-model="record.transferArea" />
            </template>
            <!-- 使用方向变更 -->
            <template v-if="changeType === '4'" slot="operationArea" slot-scope="text, record">
              <a-input-number size="small" :min="0" :step="1.0" :precision="4" v-model="record.operationArea" />
            </template>
            <template v-if="changeType === '4'" slot="selfUserArea" slot-scope="text, record">
              <a-input-number size="small" :min="0" :step="1.0" :precision="4" v-model="record.selfUserArea" />
            </template>
            <template v-if="changeType === '4'" slot="idleArea" slot-scope="text, record">
              <a-input-number size="small" :min="0" :step="1.0" :precision="4" v-model="record.idleArea" />
            </template>
            <template v-if="changeType === '4'" slot="occupationArea" slot-scope="text, record">
              <a-input-number size="small" :min="0" :step="1.0" :precision="4" v-model="record.occupationArea" />
            </template>
            <template v-if="changeType === '4'" slot="otherArea" slot-scope="text, record">
              <a-input-number size="small" :min="0" :step="1.0" :precision="4" v-model="record.otherArea" />
            </template>
            <!-- 原值变更 -->
            ...
            <template v-if="changeType === '8'" slot="newCreditorAmount" slot-scope="text, record">
              <a-input-number size="small" :min="0" :step="1.0" :precision="4" v-model="record.newCreditorAmount" />
            </template>
            <template v-if="changeType === '8'" slot="newDebtAmount" slot-scope="text, record">
              <a-input-number size="small" :min="0" :step="1.0" :precision="4" v-model="record.newDebtAmount" />
            </template>
            <template v-if="changeType === '9'" slot="newAssetArea" slot-scope="text, record">
              <a-input-number size="small" :min="0" :step="1.0" :precision="4" v-model="record.newAssetArea" />
            </template>
            <!-- 变更后实际产权单位 -->
...
export default {
  ...
  methods: {
        ...
        if (!err) {
          ...
          for (let i = 0; i < this.tableData.length; i++) {
            if (String(this.changeType) === '1') {
              ...
              if (conditionalJudgment.includes(this.tableData[i].newOriginalValue)) {
          ...
          this.tableData.forEach((item) => {
            if (
              isNaN(Number(item.newUseDirection)) &&
              String(this.changeType) === '4' &&
              String(this.assetType) === this.$store.state.ASSET_TYPE_CODE.EQUIPMENT
            ) {
            ...
            arr.push({
              ...
              transferArea: String(this.changeType) === '2' ? item.transferArea : '', // 交付物业面积-交付物业变更
              transferOperationArea: String(this.changeType) === '1' ? item.transferOperationArea : '', // 交付运营面积-交付运营变更
              operationArea: String(this.changeType) === '4' ? item.operationArea : '', // 运营面积-使用方向变更
              ...
              managementRight: String(this.changeType) === '7' ? item.managementRight : '', // 有无经营产权
              newAssetName: String(this.changeType) === '7' ? item.newAssetName : '', // 变更后资产名称
              newAssetCode: String(this.changeType) === '7' ? item.newAssetCode : '', // 变更后资产编码
              newSourceMode: String(this.changeType) === '7' ? item.newSourceMode : '', // 变更后来源方式
              decorationSituation: String(this.changeType) === '7' && ['1'].includes(String(this.assetType)) ? item.newDecorationSituation : '', // 变更后装修情况
              originSource: String(this.changeType) === '7' ? item.newOriginSource : '', // 变更后资产原始来源方
              creditorAmount: String(this.changeType) === '8' ? item.newCreditorAmount : '', // 变更后债权金额
              debtAmount: String(this.changeType) === '8' ? item.newDebtAmount : '', // 变更后债权金额
              newAssetArea: String(this.changeType) === '9' ? item.newAssetArea : '', // 变更后资产面积
              newUseDirection:
                String(this.changeType) === '4' && String(this.assetType) === this.$store.state.ASSET_TYPE_CODE.EQUIPMENT ? item.newUseDirection : '', // 变更后使用方向(设备设施 独有)
    ...
    joinRemark() {
      ...
      if (this.changeType === '3' && this.originalObjectType !== '4') {
        let originalObjectTypeName = this.originalObjectTypeDataCom.find((item) => item.value === this.originalObjectType).name;
        let originalObjectId = this.form.getFieldValue('originalObjectId') || '';
        let originalObjectIdName = this.originalObjectIdData.find((item) => item.value === originalObjectId).name;
        let shareWayName = this.shareWayData.find((item) => item.value === this.shareWay).name;
    ...
    computedEqually: debounce(function () {
      ...
      if (shareWay === '1') {
        ...
        this.tableData.forEach((item) => {
          item.newOriginalValue = !Number(item.assetArea) ? 0 : Number(Number(calc.multiply(pin, item.assetArea)).toFixed(2));
        });
        // 计算平均值总和
    ...
    originalObjectProject() {
      ...
      // let projectId = this.form.getFieldValue("projectId");
      this.originalObjectIdData = [this.projectIdData.find((item) => item.value === projectId)];
    },
    ...
    async getStallApiPageList(val) {
      ...
      const {
        data: { code, data },
      } = await this.$api.assets.carPage(requestData);
      if (code === '0') {
        let resultList = data;
    ...
    async getParkApiList() {
      ...
      const {
        data: { code, data },
      } = await this.$api.building.parkApiList(requestData);
      if (code === '0') {
