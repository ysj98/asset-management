  ...
  methods: {
    ...
    placeFun(value) {
      this.$nextTick(function () {
        this.organProjectBuildingValue.placeId = this.handleMultipleSelectValue(value, this.organProjectBuildingValue.placeId, this.buildingOptions);
      });
    },
    assetLabelFn(value) {
      this.$nextTick(function () {
    ...
    handleViewDetail(record) {
      ...
      record.placeId &&
        this.$router.push({
          ...
          query: { organId, assetIds: JSON.stringify(record.assetIds), placeId: record.placeId, organName: organName },
        });
    },
    ...
    queryTableData({ pageNo = 1, pageLength = 10, type }) {
      ...
      const {
        organProjectBuildingValue: { organId, projectId: projectIdList, placeId: placeIdList },
        current,
      } = this;
    ...
    queryBuildingList(val) {
      ...
      this.$api.carPark
        .organPlace({ organId, projectIds: projectId ? projectId.join(',') : '' })
        .then((r) => {
          // 查询当前组织机构下所有的楼栋 后面改成查询所有车场
          let res = r.data;
          if (res && String(res.code) === '0') {
