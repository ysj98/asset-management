        ...
        <a-row v-if="Object.keys(item.details).length && item.title === '接管信息'">
          <a-col :span="8" v-for="(name, key) in item.details" :key="key">
            ...
            <div style="display: inline-block" v-if="key === 'attachment' && detailData[key] && detailData[key].length > 0">
              <UploadFile type="all" :show="true" v-model="detailData[key]" />
            </div>
          </a-col>
...
export default {
  ...
  data() {
    return {
      sourceType: '1', // 来源方式 其他 99，租入 4，自建 3，外购 2，划转 1
      ...
      apiObj: {
        ...
        patrolRecord: {
          ...
          data: 'assetId',
          pagination: true,
          prower: 'power_car_patrolRecord',
        }, // 巡查记录
        accessoryInfo: {
          api: 'queryAssetViewAccessoryDetail',
          tip: '附属&配套',
          param: 'assetId',
          data: 'assetId',
          prower: 'power_car_accessoryInfo',
        }, // 附属&配套
        ...
        relatedExpenses: {
          ...
          data: 'assetId',
          pagination: true,
          prower: 'power_car_relatedExpenses',
        }, // 相关费用
        archive: {
