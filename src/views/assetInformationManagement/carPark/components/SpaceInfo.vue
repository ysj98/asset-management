...
export default {
  ...
  data() {
    return {
      ...
      spaceInfoKeys: [
        ...
        { title: '所在车场', key: 'placeName' },
        { title: '所在区域', key: 'parkingAreaName' },
  ...
  methods: {
    ...
    queryDetailInfo() {
      this.spinning = true;
      this.$api.carPark
        .assetCarPark({ assetCarParkId: this.assetCarParkId })
        .then((r) => {
          this.spinning = false;
          let res = r.data;
    ...
    handleModalOpen(type) {
      ...
      const {
        infoData: { assetName, assetCode, decorationSituation, assetCarParkId, ownerUser, ownerOrgan, ownerOrganName },
      } = this;
