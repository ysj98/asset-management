    ...
    <div class="pb70">
      <SearchContainer v-model="toggle" :contentStyle="{ paddingTop: toggle ? '16px' : 0 }">
        <div slot="headerBtns">
          ...
          <SG-Button v-power="ASSET_MANAGEMENT.ASSET_EXPORT_SUBSI" type="primary" @click="exportData"
        ...
        <div slot="headerForm" style="float: right; text-align: left">
          ...
          />
          <!-- 资产名称或编码 -->
          <a-input :maxLength="30" placeholder="输入资产名称/编码" v-model="queryCondition.assetNameOrCode" :style="allStyle" />
        </div>
        <div slot="contentForm" class="search-content-box">
          <div class="search-from-box">
            ...
            <a-select
              ...
              notFoundContent="没有查询到数据"
            />
            <a-input :maxLength="30" v-model="queryCondition.matchingNameOrCode" placeholder="附属配套名称/编码" :style="allWidth" />
          </div>
    ...
    >
      <div slot="upLoadModule" class="upLoad-content">
        ...
        <div class="upLoad-content-li">
          ...
          <a-select placeholder="请选择资产类型" v-model="upLoadInfo.assetType" :options="$addTitle(assetTypeOptions)" :style="allStyle"></a-select>
        </div>
...
export default {
  ...
  methods: {
    ...
    platformDictFn(code) {
      this.$api.assets.platformDict({ code }).then((res) => {
        if (res.data.code === '0') {
          ...
          // 资产类型
          if (code === 'asset_type') {
            this.assetTypeListOpt = [...utils.deepClone(assetTypeListOpt), ...arr];
          }
        } else {
          this.$message.error(res.data.message);
        }
    ...
    exportData() {
      ...
      this.$api.subsidiary.exportData(data).then(
        (res) => {
    ...
    downModeFile() {
      ...
      this.$api.assets.downBatchModle(params).then(
        (res) => {
          this.$SG_Message.destroy(loadingName);
          // if (Number(res.data.code) === 0) {
    ...
    uploadModeFile(file) {
      ...
      this.$api.subsidiary.batchImport(fileData).then(
        (res) => {
          if (Number(res.data.code) === 0) {
      ...
      this.$api.assets.organDict({ organId: this.queryCondition.organId, code }).then((res) => {
        if (res.data.code === '0') {
          ...
          if (code === 'SUBSIDIARY_MATCHING_TYPE') {
            this.matchingTypeListOpt = [...utils.deepClone(matchingTypeListOpt), ...arr];
          }
        } else {
          this.$message.error(res.data.message);
        }
