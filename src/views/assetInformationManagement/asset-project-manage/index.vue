  ...
  <div class="project_manage">
    ...
    <search-container v-model="fold" style="position: relative">
      <div slot="headerBtns">
        ...
        <SG-Button icon="plus" type="primary" v-power="ASSET_MANAGEMENT.ASSET_APM_NEW" @click="handleModalOpen('add', new Date().getTime())"
          >新增资产项目</SG-Button
        >
      </div>
    ...
    <a-table v-bind="tableObj" size="middle">
      ...
      <span slot="action" slot-scope="text, record">
        ...
        <span
          ...
          >审核</span
        >
        <router-link :to="{ path: '/assetProjectManage/assetProjectDetail', query: { projectId: record.projectId } }" class="action_text"
          >详情</router-link
        >
      </span>
    </a-table>
  ...
  methods: {
    ...
    handleModalOk() {
      ...
      if (type === 'add' || type === 'edit') {
        ...
        this.tableObj.loading = true;
        this.$api.assets
          .doProjectManageAudit({ projectId })
          .then((r) => {
            this.tableObj.loading = false;
            let res = r.data;
            if (res && String(res.code) === '0') {
    ...
    confirmDelete(projectId) {
      this.tableObj.loading = true;
      this.$api.assets
        .deleteProjectManageById({ projectId })
        .then((r) => {
          this.tableObj.loading = false;
          let res = r.data;
          if (res && String(res.code) === '0') {
    ...
    confirmApproval(projectId) {
      this.tableObj.loading = true;
      this.$api.assets
        .doProjectManageReaudit({ projectId })
        .then((r) => {
          this.tableObj.loading = false;
          let res = r.data;
          if (res && String(res.code) === '0') {
    ...
    queryTableData({ pageNo = 1, pageLength = 10, type }) {
      ...
      this.$api.assets
        .queryProjectManageListPage(form)
        .then((r) => {
          this.tableObj.loading = false;
          let res = r.data;
          if (res && String(res.code) === '0') {
    ...
    queryStatistics(form) {
      ...
      this.$api.assets
        .queryProjectManageProjectStatistics(form)
        .then((r) => {
