  ...
  <div class="assets-clear">
    <SG-SearchContainer size="fold" background="white" v-model="toggle" @input="searchContainerFn">
      <div slot="headBtns">
        ...
        <SG-Button @click="changeListSettingsModal(true)" v-power="ASSET_MANAGEMENT.ASSET_IN_SET_TABLE_HEADER"> 列表设置 </SG-Button>
        <SG-Button icon="plus" type="primary" @click="newClearForm" :loading="exportBtnLoading" v-power="ASSET_MANAGEMENT.ASSET_IN_EXPORT"
          >导出</SG-Button
        >
        <div style="position: absolute; top: 20px; right: 76px; display: flex">
          ...
          <a-select
            ...
            @select="changeStatus"
          ></a-select>
          <a-input v-model.trim="queryData.assetName" style="width: 190px" :maxLength="30" placeholder="资产名称/编码" />
        </div>
      </div>
      <div slot="btns">
      ...
      <div slot="form" class="formCon">
        ...
        <SG-DatePicker
          ...
          @change="onDateChange"
        ></SG-DatePicker>
        <a-input v-model.trim="queryData.cleaningOrderCode" style="width: 190px" :maxLength="30" placeholder="出库单名称/编号" />
        <div style="width: 100%">
  ...
  methods: {
    ...
    assetClassifyDataFn(value, isSelectedEquipment) {
      this.$nextTick(() => {
        ...
        this.queryData.objectTypeList = this.handleMultipleSelectValue(value, this.queryData.objectTypeList, resOptions);
      });
    },
    // 状态发生变化
    changeStatus(value) {
      this.$nextTick(() => {
        this.queryData.approvalStatusList = this.handleMultipleSelectValue(value, this.queryData.approvalStatusList, this.approvalStatusData);
      });
    },
    // 出库原因发生改变
    changeCleanupType(value) {
      this.$nextTick(function () {
        this.queryData.cleanupTypeList = this.handleMultipleSelectValue(value, this.queryData.cleanupTypeList, this.cleanupTypeData);
      });
    },
    // 资产类型发生变化
    ...
    filterOption(value) {
      this.$nextTick(() => {
        this.queryData.projectIdList = this.handleMultipleSelectValue(value, this.queryData.projectIdList, this.assetProjectOptions);
      });
    },
    // 处理多选下拉框有全选时的数组
    ...
    handleSearchReq(isTotal) {
      ...
      let obj = {};
      ['assetTypeList', 'objectTypeList', 'cleanupTypeList', 'approvalStatusList', 'projectIdList'].forEach(
        (key) =>
          // 过滤空值
