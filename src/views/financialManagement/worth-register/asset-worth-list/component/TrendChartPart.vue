...
<template>
  <a-spin :spinning="spinning">
    <div class="trend_chart">
      ...
      <div class="tip_style">
        <span
          >资产原值：<span style="font-weight: bold">{{ originalValue || '--' }}</span
          >元</span
        >
        <span v-for="m in tips" :key="m.title"> <i :style="{ backgroundColor: m.color }"></i>{{ m.title }} </span>
      </div>
    </div>
  </a-spin>
</template>
  ...
  methods: {
    ...
    renderChart(dataSource) {
      ...
      let data = dataSource.map((m) => {
        ...
        return {
          ...
          tooltip: {
            padding: [10, 15],
            formatter: `评估价值：${assessmentValue}元<br/>评估方法：${
              assessmentMethodName || ''
            }<br/>评估机构：${assessmentOrganName}<br/>评估时间：${assessmenBaseDate}`,
          },
        };
      });
      // 初始化echarts实例
    ...
    queryTrendInfo() {
      ...
      this.$api.worthRegister
        .queryAssetValueTrend({ assetId: this.assetId })
        .then((r) => {
          this.spinning = false;
          let res = r.data;
