...
<template>
  <div class="asset_worth_register">
    ...
    <search-container v-model="fold">
      <div slot="headerBtns">
        <SG-Button icon="plus" type="primary" v-power="ASSET_MANAGEMENT.ASSET_AWR_ADD" @click="handleBtnAction({ type: 'add' })"
          >新建价值登记单</SG-Button
        >
        <SG-Button
          icon="export"
    ...
    <!--列表部分-->
    <a-table v-bind="tableObj" class="td-pd10">
    ...
    return {
      ...
      tableObj: {
        ...
        columns: [
          ...
          { title: '价值登记单名称', dataIndex: 'registerName', width: 120, ellipsis: true },
          { title: '资产项目', dataIndex: 'projectName', ellipsis: true },
          { title: '资产类型', dataIndex: 'assetTypeName', ellipsis: true },
          { title: '评估方法', dataIndex: 'assessmentMethodName', width: 160, ellipsis: true },
          { title: '评估机构', dataIndex: 'assessmentOrganName', width: 180, ellipsis: true },
          { title: '评估基准日', dataIndex: 'assessmenBaseDate', ellipsis: true },
          { title: '资产数量', dataIndex: 'num', ellipsis: true },
          { title: '提交人', dataIndex: 'createByName', ellipsis: true },
          { title: '提交时间', dataIndex: 'createTime', ellipsis: true },
          { title: '状态', dataIndex: 'approvalStatusName', ellipsis: true },
          {
            title: '附件',
            key: 'uploadAttachment',
            customRender(text, { uploadAttachment }) {
              return uploadAttachment ? '已上传' : '未上传';
            },
            ellipsis: true,
          },
  ...
  methods: {
    ...
    querySumInfo(form) {
      this.$api.worthRegister
        .queryValueRegisterPageListSum(form)
        .then(({ data: res }) => {
    ...
    handleBtnAction({ id, type }) {
      if (type === 'add') {
        const {
          organProjectType: { organId, organName },
        } = this;
    ...
    confirmDelete(registerId) {
      ...
      this.$api.worthRegister
        .updateStatus({ registerId, approvalStatus: 4 })
        .then((r) => {
