    ...
    <div style="margin-left: 40px">
      ...
      <div style="margin-bottom: 8px; text-align: right">
        ...
        <span v-if="type == 'add' || type == 'edit'">
          ...
          <SG-Button icon="plus" type="primary" ghost @click="handleAddCard(true)">添加资产卡片</SG-Button>
          <!--<SG-Button icon="import" type="primary" ghost @click="handleExport" :loading="exportBtnLoading">批量导入</SG-Button>-->
          <SG-Button icon="delete" type="primary" ghost @click="handleDelete" style="margin-left: 10px">删除</SG-Button>
        </span>
...
export default {
  ...
  methods: {
    ...
    // 资产卡片返回的资产id
    assetCardSubmit(selectedRowKeys) {
      selectedRowKeys = Array.from(new Set(selectedRowKeys));
      this.selectedList = selectedRowKeys;
      this.initList = [...selectedRowKeys];
      this.queryAssetListByAssetId(selectedRowKeys);
    },
    ...
    queryAssetListByRegisterId({ pageNo = 1, pageLength = 10, type }) {
      ...
      this.$api.worthRegister
        .queryRelPageList({ registerId, pageSize: pageLength, pageNum: pageNo })
        .then((r) => {
          this.tableObj.loading = false;
          let res = r.data;
          if (res && String(res.code) === "0") {
