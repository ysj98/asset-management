...
6.initValue: Array，初始值,默认格式['UID001', 'UID001']，如果 allAttrs 为true, 传入格式为[{userId: 'UID001'}]
-->
...
<template>
  <div>
    <a-modal
      ...
      @ok="handleAction('ok')"
    >
      <select-asset-list v-model="selectedList" :height="height" :organId="organId"/>
    </a-modal>
  </div>
</template>
<script>
  ...
  export default {
    ...
    props: {
      ...
      // 是否获取全部属性，默认false获取userId字符串构成的数组
      allAttrs: {
        type: Boolean,
        default: () => true
      },
