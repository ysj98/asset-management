  ...
  <div class="particulars">
      ...
      <div class="particulars-obj">
        <a-row class="playground-row">
          ...
          <a-col class="playground-col" :span="8"
            >计划执行时间：{{ particularsData.endDate ? `${particularsData.beginDate} - ${particularsData.endDate}` : '--' }}</a-col
          >
    ...
    <div class="particulars-nav">
      <span class="section-title blue"
        >盘点资产清单（资产总数:{{ inventoryAssetCount || 0 }}；已盘点:{{ inventoryCheckCount || 0 }}; 未盘点:{{ inventoryNoCheckCount || 0 }})</span
      >
      <div class="particulars-obj">
        <Cephalosome style="margin: 0" :rightCol="18" :leftCol="6">
          <div slot="col-l">
            <SG-Button
              v-power="ASSET_MANAGEMENT.ASSET_EXECUTE_LISTASSETS"
              type="primary"
              style="margin-right: 10px"
              weaken
              @click="downloadTemplateFn('0')"
              >导出资产清单</SG-Button
            >
          ...
          <div slot="col-r" class="nav">
            <a-select
              ...
              :tokenSeparators="[',']"
              v-model="queryCondition.checkStatus"
            >
              <a-select-option :title="item.name" v-for="(item, index) in checkStatusData" :key="index" :value="item.value">{{
                item.name
              }}</a-select-option>
            </a-select>
    ...
    <div class="particulars-nav">
      <span class="section-title blue"
        >盘点异常列表（异常总数:{{ exceptionCount || 0 }}；盘亏:{{ failCount || 0 }}; 盘盈:{{ successCount || 0 }}; 信息有误:{{
          errorCount || 0
        }})</span
      >
      <div class="particulars-obj">
        <Cephalosome style="margin: 0" :rightCol="18" :leftCol="6">
          <div slot="col-l">
            ...
            <SG-Button v-power="ASSET_MANAGEMENT.ASSET_EXECUTE_EXCEPTION" type="primary" @click="downloadTemplateFn('1')" weaken
              >导出异常信息</SG-Button
            >
          </div>
          <div slot="col-r">
            <div class="nav">
              <a-select style="width: 160px" placeholder="全部异常状态" @change="checkResultsChange" v-model="condition.checkResults">
                <a-select-option :title="item.name" v-for="(item, index) in checkResultsData" :key="index" :value="item.value">{{
                  item.name
                }}</a-select-option>
              </a-select>
            </div>
        ...
        <div class="table-layout-fixed table-border">
          <a-table :loading="tableLoading" :columns="exceptionListColumns" :dataSource="tableDataList" size="middle" :pagination="false">
            <template slot="operation" slot-scope="text, record">
              <div v-if="changeType === 'set'">
    ...
    <div class="particulars-nav" v-if="changeType === 'set'">
      ...
      <div class="particulars-obj" style="line-height: 64px; padding: 20px 0 30px 0">
        <span class="required-color">盘点结果：</span
        ><a-textarea
  ...
  methods: {
    ...
    save() {
      ...
      _this.$confirm({
        title: '提示',
        content:
          +this.inventoryNoCheckCount !== 0
            ? `存在${this.inventoryNoCheckCount}个未盘点的资产，提交后将不能再修改了，确认要提交该盘点单吗？`
            : '提交后将不能再修改了，确认要提交该盘点单吗？',
        onOk() {
    ...
    change(files, e) {
      ...
      this.$api.inventoryManagementApi.importExcel(this.formData).then(
        (res) => {
          if (res.data.code === '0') {
            e.target.value = '';
            this.DE_Loding(loadingName).then(() => {
