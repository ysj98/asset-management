  ...
  <div class="plan-page pb70">
    ...
    <div class="search-box">
      <div class="search-left">
        <SG-Button v-power="ASSET_MANAGEMENT.zcgl_plan_create" icon="plus" @click="goPage('create')" type="primary">新增盘点计划</SG-Button>
      </div>
      <div class="search-right">
        ...
        <div style="margin-top: 20px">
          <SG-DatePicker v-model="queryCondition.beginDate" :style="allWidth" @change="onSearch()" label="生效日期"></SG-DatePicker>
        </div>
        <div style="margin-top: 20px">
          <SG-DatePicker v-model="queryCondition.endDate" :style="allWidth" @change="onSearch()" label="失效日期"></SG-DatePicker>
        </div>
        <!-- 输入框 -->
        <a-input-search :style="allStyle" v-model="queryCondition.planName" placeholder="计划名称" :maxLength="30" @search="onSearch" />
      </div>
    </div>
    <!-- 搜索框end -->
    <div>
      ...
      if (['delete', 'readApproval'].includes(type)) {
        ...
        this.$SG_Modal.confirm({
          ...
          onOk: () => {
            ...
            this.$api.building
              .updateCheckPlanStatus({ approvalStatus, planId: record.planId })
              .then((res) => {
