...
<template>
  <a-spin :spinning="spinning">
    <div class="inventoryPlan-create">
        ...
        <div class="edit-box">
          ...
          <div class="edit-box-content">
            ...
            <div class="edit-box-content-item total-width">
              <div class="label-name-box">
                <span class="label-name label-space-between">备注<i></i></span><span>：</span>
              </div>
            ...
            <div class="edit-box-content-item total-width">
              <div class="label-name-box">
                <span class="label-name label-space-between">附件<i></i></span><span>：</span>
              </div>
              <a-form-item class="label-value">
      ...
      <div class="table-layout-fixed detail-table ml40">
        <a-table size="middle" :loading="table.loading" :pagination="false" :columns="table.columns" :dataSource="table.dataSource">
          ...
          <template slot="chargePersonNameList" slot-scope="text, record, index">
            ...
            <span v-else>{{ record.chargePersonNameList }}</span>
          </template>
          ...
          <template slot="operation" slot-scope="text, record, index">
            <span class="btn-delete" @click="deleteTableLine(record, index)">删除</span>
          </template>
        </a-table>
      </div>
      ...
      <FormFooter v-if="['create', 'edit'].includes(pageType)" style="border: none" location="fixed">
        <SG-Button class="mr2" @click="handleSaveApp('0')" type="primary">保存草稿</SG-Button>
        <SG-Button @click="handleSaveApp('1')">提交审批</SG-Button>
      </FormFooter>
      <FormFooter v-if="['approval'].includes(pageType)" style="border: none" location="fixed">
        <SG-Button class="mr2" @click="handleAdopt('1')" type="primary">审批通过</SG-Button>
...
export default {
  ...
  methods: {
    ...
    inventoryDetail() {
      this.spinning = true;
      this.$api.building
        .inventoryDetail({ planId: this.planId })
        .then((res) => {
          if (res.data.code === '0') {
    ...
    handleAdopt(approvalStatus) {
      ...
      this.$api.building
        .updateCheckPlanStatus({ approvalStatus, planId: this.planId })
        .then((res) => {
          this.DE_Loding(loadingName).then(() => {
            if (res.data.code === '0') {
