              ...
              <a-form-item v-bind="formItemLayout" label="任务名称：">
                <a-input
                  ...
                  v-decorator="[
                    'taskName',
                    {
                      rules: [{ required: true, max: 50, whitespace: true, message: '请输入任务名称(不超过50字符)' }],
                      initialValue: newCardData.taskName,
                    },
                  ]"
                />
              </a-form-item>
            ...
            <a-col class="playground-col" :span="8">
              <a-form-item v-bind="formItemLayout" label="负责人：">
                <a-select
                  ...
                  v-decorator="[
                    'chargePerson',
                    { rules: [{ required: true, whitespace: true, message: '请选择负责人' }], initialValue: newCardData.chargePerson },
                  ]"
                >
              ...
              <a-form-item v-bind="formItemLayout" label="执行时间：">
                <SG-DatePicker
                  ...
                  v-decorator="[
                    'defaultValue',
                    {
                      rules: [{ type: 'array', required: true, whitespace: true, message: '请选择执行时间' }],
  ...
  methods: {
    ...
    chanheFn(val, index, str) {
      ...
      if (beginDateNew > valDate || valDate > endDateNew) {
        str === 'beginDate'
          ? (this.table.dataSource[index].beginDate = this.form.getFieldValue('defaultValue')[0])
          : (this.table.dataSource[index].endDate = this.form.getFieldValue('defaultValue')[1]);
      ...
      this.$api.inventoryManagementApi.queryListByTaskId(obj).then((res) => {
        if (Number(res.data.code) === 0) {
          ...
          data.forEach((item, index) => {
            ...
            (item.chargePerson = item.userIds), (item.chargePersonOpt = [{ label: item.userNames, key: item.userIds }]);
