        ...
        <div class="edit-box-content">
          <div class="edit-box-content-item">
            <div class="label-name-box" :class="{ required: editable }">
              <span class="label-name" :class="{ 'label-space-between': editable }">所属盘点任务<i></i></span><span>：</span>
            </div>
            <a-form-item>
              <a-select
                :style="allStyle"
                ...
                v-if="editable"
                v-decorator="['taskId', { rules: [{ required: true, message: '请选择关联资产' }], initialValue: detail.taskId }]"
              >
          ...
          <div class="edit-box-content-item">
            <div class="label-name-box" :class="{ required: editable }">
              <span class="label-name" :class="{ 'label-space-between': editable }">盘点报告名称<i></i></span><span>：</span>
            </div>
            <a-form-item>
              <a-input
                ...
                v-decorator="[
                  'reportName',
                  {
                    rules: [{ required: true, max: 30, whitespace: true, message: '请输入盘点报告名称(不超过30字符)' }],
                    initialValue: detail.reportName,
                  },
                ]"
              />
          ...
          <div class="edit-box-content-item total-width" v-show="detail.taskId">
            <div class="label-name-box">
              <span class="label-name" :class="{ 'label-space-between': editable }">盘点结果描述<i></i></span><span>：</span>
            </div>
            <a-form-item class="label-value">
              <span class="label-value"
                >盘点资产总数{{ assetListCount.total }}个，正常资产{{ assetListCount.normal }}个，未盘点{{ assetListCount.unCount }}个，异常资产{{
                  assetListCount.abnormal
                }}个（盘亏{{ assetListCount.fail }}，盘盈{{ assetListCount.success }}，信息有误{{ assetListCount.error }}）</span
              >
            </a-form-item>
          </div>
          <div class="edit-box-content-item" v-show="!editable">
            <div class="label-name-box">
              <span class="label-name">提交人<i></i></span><span>：</span>
            </div>
            <a-form-item>
          ...
          <div class="edit-box-content-item" v-show="!editable">
            <div class="label-name-box">
              <span class="label-name">提交时间<i></i></span><span>：</span>
            </div>
            <a-form-item>
          ...
          <div class="edit-box-content-item" v-show="!editable">
            <div class="label-name-box">
              <span class="label-name">状态<i></i></span><span>：</span>
            </div>
            <a-form-item>
          ...
          <div class="edit-box-content-item total-width">
            <div class="label-name-box">
              <span class="label-name" :class="{ 'label-space-between': editable }">总结说明<i></i></span><span>：</span>
            </div>
            <a-form-item class="label-value">
              <a-textarea
        ...
        <div class="edit-box-content">
          <a-tabs type="card" :tabBarGutter="10">
            <a-tab-pane :tab="'异常资产(' + assetListCount.abnormal + ')'" key="1">
              <div class="header-container">
                <div class="count-description">
                  异常总数：{{ assetListCount.abnormal }}，盘亏{{ assetListCount.fail }}，盘盈{{ assetListCount.success }}，信息有误{{
                    assetListCount.error
                  }}
                </div>
                <div class="abnormal-type">
                  <a-checkbox
                    :value="item.value"
                    :key="index"
                    :checked="abnormalAsset.abnormalType.indexOf(item.value) > -1"
                    :disabled="abnormalAsset.abnormalType.indexOf(item.value) > -1 && abnormalAsset.abnormalType.length === 1"
                    v-for="(item, index) in abnormalAsset.abnormalTypeOptions"
                    @change="onAbnormalTypeChange"
              ...
              <div class="tab-nav">
                <div class="table-border table-layout-fixed">
                  <a-table :columns="abnormalAsset.columns" :dataSource="abnormalAsset.dataSource" size="middle" :pagination="false">
            ...
            <a-tab-pane :tab="'未盘点资产(' + assetListCount.unCount + ')'" key="2">
              <div class="tab-nav">
                <div class="table-border table-layout-fixed">
                  <a-table :columns="unCountAsset.columns" :dataSource="unCountAsset.dataSource" size="middle" :pagination="false"> </a-table>
                  <SG-FooterPagination
            ...
            <a-tab-pane :tab="'正常资产(' + assetListCount.normal + ')'" key="3">
              <div class="tab-nav">
                <div class="table-border table-layout-fixed">
                  <a-table :columns="normalAsset.columns" :dataSource="normalAsset.dataSource" size="middle" :pagination="false">
                    <template slot="operation" slot-scope="text, record">
                      <div>
                        <span @click="viewDetails(record)" class="btn_click">详情</span>
                      </div>
