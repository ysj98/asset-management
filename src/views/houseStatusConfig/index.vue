<template>
  <div class="status-config">
    ...
    <a-table
     ...
     :pagination="false"
     >
     <div slot="isAble" slot-scope="text, record"><a-switch :checked="record.isAble === 'Y' " @click="handleChange(record)"></a-switch>{{ text === 'Y' ? '已开启' : '已禁用' }}</div>
     <div slot="alias" slot-scope="text, record"><a-input v-model.trim="record.alias" /></div>
     <div slot="color" slot-scope="text, record, index" >{{record.color}} <span class="color-btn" @click.stop="show=true; active=index" :style="{backgroundColor: record.color}"></span>
