...
<template>
  <div class="landInfo-page pb70">
    ...
    <edit-dialog :visible="editVisible" :object="selectItem" @close="() => (editVisible = false)" @submit="handleEditSubmit" />
    <export-doalog
...
import { allStyle, columns, queryCondition, approvalStatusData } from './dict.js';
...
export default {
  ...
  data() {
    return {
      ...
      numList: [], // 概览数字数据, title 标题，value 数值，bgColor 背景色
      table: {
        columns: [],
        dataSource: [{}],
        loading: false,
  ...
  watch: {
    $route() {
      if (this.$route.path === '/buildingDict' && this.$route.query.refresh && this.$route.query.showKey === 'equipment') {
  ...
  methods: {
    ...
    uploadFile(file) {
      ...
      this.$api.assetBussinessInformation
        .readExcelModel(fileData)
        .then((r) => {
