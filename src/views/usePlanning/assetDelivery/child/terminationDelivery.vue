...
-->
<template>
  <SG-Modal v-model="status" title="结束交付" width="600px" @ok="submit" :noPadding="true" @cancel="cancel">
    <div class="p15">
      <a-form :form="form">
        <a-form-item label="结束日期：" v-bind="formItemLayout">
          <a-date-picker v-decorator="['endDate', { rules: [{ required: true, message: '请选择日期' }], initialValue: subData.endDate }]" />
        </a-form-item>
        <a-form-item label="原因：" v-bind="formItemTextarea">
          <a-textarea
            ...
            v-decorator="['cause', { rules: [{ required: false, max: 200, message: '请输入原因(不超过200字符)' }], initialValue: subData.cause }]"
          />
        </a-form-item>
      </a-form>
    </div>
