  ...
  <div>
    <SG-SearchContainer background="white">
      <div slot="headBtns">
        ...
        <SG-Button icon="export" :loading="exportBtnLoading" style="margin-right: 8px" @click="downloadFn">导出</SG-Button>
      </div>
  ...
  methods: {
    ...
    operationFun(record) {
      this.$router.push({
        ...
        query: { warrantNbr: record.warrantNbr, kindOfRightName: record.kindOfRightName, logId: record.logId },
      });
    },
    creatDebt() {
    ...
    editDebt(record) {
      this.$router.push({
        path: '/debt/editDebt',
        query: { organId: this.queryCondition.organId, organName: this.organName, setType: 'edit', debtId: record.debtId },
      });
    },
    detailDebt(record) {
      this.$router.push({
        path: '/debt/detailDebt',
        query: { organId: this.queryCondition.organId, organName: this.organName, setType: 'detail', debtId: record.debtId },
      });
    },
    deleteDebt(record) {
      let _this = this;
