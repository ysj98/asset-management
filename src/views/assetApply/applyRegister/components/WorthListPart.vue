...
<template>
  <div class="worth_list">
    ...
    <div style="margin-left: 40px">
      <div style="margin-bottom: 8px; text-align: right">
        <div v-if="type == 'add' || type == 'edit'" class="box">
          <div class="left" style="height: 100%">
            已选择资产数量：{{ tableObj.dataSource.length }}
            <span v-if="type == 'add' && !isSelectedEquipment">，合计领用面积：{{ receiveAreaSum }}㎡</span>
          </div>
      ...
      <div class="sum" style="margin-left: 980px" v-if="type == 'detail' && !isSelectedEquipment">合计：{{ receiveAreaSum }}㎡</div>
    ...
    <p style="margin-left: 1160px" v-if="type == 'edit' && !isSelectedEquipment">合计：{{ receiveAreaSum }}㎡</p>
...
export default {
    ...
    return {
      tableObj: {
        ...
        columns: [
          ...
          { slots: { title: 'receiveArea' }, dataIndex: 'receiveArea', scopedSlots: { customRender: 'receiveArea' } },
  ...
  methods: {
    ...
    queryAssetListByRegisterId({ pageNo = 1, pageLength = 10, type }) {
      ...
      this.$api.useManage
        .getReceiveAssetDetailPage({ receiveId: registerId, pageNum: pageNo, pageSize: pageLength })
        .then((r) => {
    ...
    queryAssetListByAssetId(selectedRows = [], status) {
      ...
      this.tableObj.loading = true;
      this.$api.useManage
        .getReceiveInfo({ receiveId: registerId, queryType: 1 })
        .then((r) => {
          this.tableObj.loading = false;
          let res = r.data;
          if (res && String(res.code) === '0') {
  ...
  watch: {
    ...
    dynamicData: function (data, oldData) {
      ...
      if (
        data.receiveOrganId != oldData.receiveOrganId ||
        data.receiveUserId != oldData.receiveUserId ||
        data.receiveDate != oldData.receiveDate ||
        data.returnDate != oldData.returnDate
      ) {
        return;
      }
      let {
