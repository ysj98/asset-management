...
<template>
  <div class="assetRegister">
    <SG-SearchContainer size="fold" background="white" v-model="toggle" @input="searchContainerFn">
      <div slot="headBtns">
        <SG-Button type="primary" v-power="ASSET_MANAGEMENT.APPLY_FORM_NEW" @click="handleBtnAction({ type: 'add' })" v-if="organName.length > 0"
          >领用登记</SG-Button
        >
        <div class="box" style="margin-left: 16px">
          <SG-Button type="primary" v-power="ASSET_MANAGEMENT.ASSET_APPLY_REGISTER" @click="exportFn"
        ...
        <div style="position: absolute; top: 20px; right: 76px; display: flex">
          ...
          <a-input-search
            v-model="queryCondition.receiveName"
            placeholder="领用单名称/编码"
            :maxLength="30"
  ...
  methods: {
    ...
    operationFun(type, record) {
      ...
      if (['edit'].includes(type)) {
        ...
        this.$router.push({
          name: '领用登记审批',
          params: { registerId: record.receiveId, type: 'approval', organId: record.organId, organName: record.organName, queryType: 1 },
        });
      } else if (['detail'].includes(type)) {
        this.$router.push({
          name: '领用登记详情',
          params: { registerId: record.receiveId, type: 'detail', organId: record.organId, organName: record.organName, queryType: 1 },
        });
      } else {
        let that = this;
        this.$confirm({
          ...
          onOk() {
            that.loading = true;
            that.$api.useManage
              .deleteReceive({ receiveId: record.receiveId })
              .then((r) => {
                that.loading = false;
                let res = r.data;
    ...
    handleBtnAction({ id, type }) {
      ...
      this.$router.push({
        name: '领用登记新增',
        params: { organId: this.queryCondition.organId, organName, type: 'add', registerId: this.queryCondition.receiveId },
      });
    },
    exportFn() {
      let obj = {
    ...
    confirmDelete(registerId) {
      this.loading = true;
      this.$api.useManage
        .deleteReceive({ receiveId: registerId })
        .then((r) => {
          this.loading = false;
          let res = r.data;
          if (res && String(res.code) === '0') {
