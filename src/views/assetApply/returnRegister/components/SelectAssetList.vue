...
<template>
  <div class="select-detail">
    <a-row :gutter="8" style="margin-bottom: 10px">
      ...
      <a-col :span="6">
        <a-input-search v-model.trim="receiveDetailId" style="width: 100%" @search="fetchData" @pressEnter="fetchData" placeholder="请输入领用编号" />
      </a-col>
      <a-col :span="6">
...
export default {
  ...
  methods: {
    ...
    fetchData({ pageLength = 10, pageNo = 1 }) {
      ...
      console.log(123);
      return this.$api.useManage
        .getReceiveRecordPage(form)
        .then((r) => {
          let res = r.data;
  ...
  mounted() {
    ...
    if (
      Object.keys(this.$store.state.ASSET_TYPE_CODE)
        .map((e) => this.$store.state.ASSET_TYPE_CODE[e])
        .includes(this.assetType)
    ) {
      this.columns = [
        { title: '领用编号', dataIndex: 'receiveDetailId', fixed: 'left', width: 120 },
        { title: '资产编码', dataIndex: 'assetCode' },
