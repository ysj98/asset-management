...
<template>
  <div class="assetRegister">
    <SG-SearchContainer size="fold" background="white" v-model="toggle" @input="searchContainerFn">
      <div slot="headBtns">
        <SG-Button type="primary" v-power="ASSET_MANAGEMENT.RETURN_FORM_NEW" @click="handleBtnAction({ type: 'add' })" v-if="organName.length > 0"
          >归还登记</SG-Button
        >
        <div class="box" style="margin-left: 16px">
          <SG-Button type="primary" v-power="ASSET_MANAGEMENT.ASSET_RETURN_REGISTER" @click="exportFn"
        ...
        <div style="position: absolute; top: 20px; right: 76px; display: flex">
          ...
          <a-input-search
            v-model="queryCondition.returnName"
            placeholder="归还单名称/编码"
            :maxLength="30"
  ...
  methods: {
    ...
    operationFun(type, record) {
      ...
      if (['edit'].includes(type)) {
        ...
        this.$router.push({
          name: '归还登记审批',
          params: { registerId: record.returnId, type: 'approval', organId: record.organId, organName: record.organName, queryType: 1 },
        });
      } else if (['detail'].includes(type)) {
        this.$router.push({
          name: '归还登记详情',
          params: { registerId: record.returnId, type: 'detail', organId: record.organId, organName: record.organName, queryType: 1 },
        });
      } else {
        let that = this;
        this.$confirm({
          ...
          onOk() {
            that.loading = true;
            that.$api.useManage
              .deleteReturn({ returnId: record.returnId })
              .then((r) => {
                that.loading = false;
                let res = r.data;
    ...
    queryInit() {
      ...
      let obj = {
        ...
        startReturnDate: this.applyValue[0] ? moment(this.applyValue[0]).format('YYYY-MM-DD') : null, // 归还开始日期
        endReturnDate: this.applyValue[1] ? moment(this.applyValue[1]).format('YYYY-MM-DD') : null, // 归还结束日期
    ...
    confirmDelete(registerId) {
      this.loading = true;
      this.$api.useManage
        .deleteReturn({ returnId: registerId })
        .then((r) => {
          this.loading = false;
          let res = r.data;
          if (res && String(res.code) === '0') {
