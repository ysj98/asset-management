...
<template>
  <div class="ownership_card">
    ...
    <a-table v-bind="tableObj" class="td-pd10 custom-scroll custom-total" ref="table">
      ...
      <template slot="warrantNbr" slot-scope="text, record, index">
        <span
          ...
          :title="record.warrantNbr"
          >{{ record.warrantNbr }}</span
        >
    ...
    <SG-FooterPagination
      ...
      @change="({ pageNo, pageLength }) => queryTableData({ pageNo, pageLength, searchType: 'page' })"
    />
    <!-- 复用权证管理查看详情页面 -->
...
export default {
  ...
  methods: {
    ...
    // 查询平台权属类型字典
    queryType() {
      queryPlatformDict('AMS_KIND_OF_RIGHT').then((list) => (this.typeOptions = [{ title: '全部权属类型', key: '' }, ...list]));
    },
