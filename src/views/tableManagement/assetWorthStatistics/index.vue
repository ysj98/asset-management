...
<template>
  <div class="asset_worth">
    ...
    <search-container size="fold" v-model="fold">
      <div slot="headerBtns">
        <SG-Button icon="import" type="primary" @click="handleExport" :loading="exportBtnLoading" v-power="ASSET_MANAGEMENT.TM_AW_EXPORT">
          导出
        </SG-Button>
        <SG-Button v-power="ASSET_MANAGEMENT.TM_AW_HEADERS_SETTING" icon="setting" @click="handleModalStatus" style="margin-left: 10px"
          >列表设置</SG-Button
        >
      </div>
...
export default {
  ...
  methods: {
    ...
    getColumns() {
      ...
      this.$api.global
        .assetRolListV2({ funType: funType })
        .then(
          ({
            data: {
              data: { customChose, customShow, templeCode },
              code,
            },
          }) => {
            if (code === '0') {
    ...
    handleModalOk() {
      ...
      // })
      this.$api.global
        .addCustomShowV2({ chooseList: req })
        .then((res) => {
          if (res.data.code === '0') {
                ...
                return {
                  ...
                  customRender: (text, row, i) => {
                    ...
                    return {
                      children: text,
                      attrs: {
                        rowSpan: temp[`${keyName}_start`] === i ? temp[keyName] : 0,
                        colSpan: row.projectName === '当前页-合计' || row.projectName === '所有页-合计' ? 0 : 1,
                      },
                    };
                  },
                };
                ...
                return {
                  ...
                  customRender: (text, row, i) => {
                    ...
                    return {
                      ...
                      attrs: {
                        rowSpan: temp[`${keyName}_start`] === i ? temp[keyName] : 0,
                        colSpan: row.projectName === '当前页-合计' || row.projectName === '所有页-合计' ? 3 : 1,
                      },
                    };
                  },
                };
                ...
                return {
                  ...
                  customRender: (text, row, i) => {
                    ...
                    return {
                      ...
                      attrs: {
                        rowSpan: temp[`${keyName}_start`] === i ? temp[keyName] : 0,
                        colSpan: row.projectName === '当前页-合计' || row.projectName === '所有页-合计' ? 2 : 1,
                      },
                    };
                  },
                };
    ...
    async queryTableData({ pageNo = 1, pageLength = 10, type }) {
      ...
      const {
        ...
        queryObj: { status, assetType, ...others },
      } = this;
      if (!organId) {
      ...
      try {
        ...
        let {
          data: {
            data: { valueOfYearCount },
          },
        } = await this.$api.tableManage.getAssetValueStatistics(form);
    ...
    generateEndTimeOption() {
      const {
        queryObj: { queryType, startTime, endTime },
      } = this;
      if (queryType === '0') {
        return (this.queryObj.endTime = undefined);
      }
      let startYear = Number(startTime);
      let obj = {
        1: 2, // 按季度统计时，最大年份跨度2年
