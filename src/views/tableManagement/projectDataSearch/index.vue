  ...
  <div>
    ...
    <search-container size="fold" v-model="fold">
      <div slot="headerBtns">
        ...
        <SG-Button icon="setting" @click="changeListSettingsModal(true)" v-power="ASSET_MANAGEMENT.TM_AP_HEADERS_SETTING" style="margin-left: 10px"
          >列表设置</SG-Button
        >
      </div>
      <div slot="headerForm" style="margin-right: 8px; text-align: left">
        <a-row :gutter="8">
          <a-col :span="13" :offset="1">
            <organ-project v-model="organProjectValue" :isShowBuilding="false" mode="multiple" :showSearch="true" :multiple="true" />
          </a-col>
          <a-col :span="3">
      ...
      <div slot="contentForm" style="margin-top: 15px">
        <a-row :gutter="8" style="width: 100%">
          <a-col :span="4" :offset="3">
            <a-select
              v-model="queryObj.transferToOperation"
              style="width: 100%"
              placeholder="请选择转运营状态"
              :options="$addTitle(operateOptions)"
            />
          </a-col>
          <a-col :span="4">
    ...
    <a-table v-bind="tableObj" class="custom-scroll custom-total td-pd10" ref="table">
      ...
      <span slot="projectName" slot-scope="text, record, index">
        <router-link
          :title="text"
          v-if="index < tableObj.dataSource.length - 2"
          :to="{ path: '/projectData/assetProjectDetail', query: { projectId: record.projectId } }"
        >
          {{ text }}
        </router-link>
        <span v-else>
