          ...
          <a-row>
            <a-col v-bind="formSpan">
              <a-form-item label="单元名称" v-bind="formItemLayout">
                <a-input
                  ...
                  v-decorator="[
                    'unitName',
                    { initialValue: '' || undefined, rules: [{ required: true, whitespace: true, message: '请输入单元名称' }] },
                  ]"
                />
              </a-form-item>
            </a-col>
            ...
            <a-col v-bind="formSpan">
              <a-form-item label="电梯数量" v-bind="formItemLayout">
                <a-input-number
                  :style="allWidth"
                  v-decorator="['liftNum', { initialValue: '' || undefined, rules: [{ pattern: /^[0-9]*$/, message: '请输入整数' }] }]"
                />
              </a-form-item>
            </a-col>
            <a-col v-bind="formSpan">
            ...
            <a-col :span="24">
              ...
              <a-form-item label="描述" v-bind="formItemLayout2">
                <a-textarea
                  @input="
                    (e) => {
                      unitDescNum = e.target.value.length;
                    }
                  "
                  v-decorator="['unitDesc', { initialValue: '' }]"
                  :maxLength="200"
                  placeholder="请输入描述内容(200字内)"
                />
          ...
          if (this.type === 'create') {
            ...
            this.$api.building
              ...
              .then((communityId) => {
                ...
                this.$api.building.addUnit(data).then(
                  (res) => {
          ...
          if (this.type === 'edit') {
            ...
            this.$api.building
              ...
              .then((communityId) => {
                ...
                this.$api.building.updateUnit(data).then(
                  (res) => {
                    this.DE_Loding(loadingName).then(() => {
                      if (res.data.code === '0') {
