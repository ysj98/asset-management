<template>
  <div class="landInfo-create-page">
        ...
        <a-row>
          ...
          <a-col :span="8">
            <a-form-model-item label="车场名称" :required="true" prop="placeName">
              <a-input style="width: 100%" :maxLength="128" placeholder="请输入车场名称" v-model="formInfo.placeName" />
            </a-form-model-item>
          </a-col>
          <a-col :span="8">
            <a-form-model-item label="车场编码" :required="true" prop="placeCode">
              <a-input :style="allWidth" :maxLength="64" placeholder="请输入车场编码" v-model="formInfo.placeCode" />
            </a-form-model-item>
          </a-col>
        </a-row>
        <a-row>
          ...
          <a-col :span="8">
            <a-form-model-item :required="true" prop="placeArea">
              ...
              <a-input-number :min="0" step="0.0001" :max="9999999999999.9999" :style="allWidth" v-model="formInfo.placeArea" />
            </a-form-model-item>
          </a-col>
          <a-col :span="8">
            <a-form-model-item label="车位数量" :required="true" prop="placeNums">
              <a-input-number step="1" :min="0" :precision="1" :style="allWidth" :max="99999999999" v-model="formInfo.placeNums" />
            </a-form-model-item>
          </a-col>
        </a-row>
        <a-row>
          ...
          <a-col :span="8">
            <a-form-model-item label="车场类型" :required="true" prop="typeId">
              <dict-select placeholder="请选择车场类型" menu-code="PARKING_PLACE_RESOURCE_TYPE" v-model="formInfo.typeId" />
            </a-form-model-item>
          </a-col>
        </a-row>
        <a-row>
          <a-col :span="24" style="display: flex">
            <!-- 文本框 -->
            <div style="width: 5.6%; text-align: right; line-height: 40px; padding-right: 10px; font-size: 12px; color: rgba(0, 0, 0, 0.85)">
              备注:
            </div>
            <div style="width: 86%">
              <a-textarea :maxLength="128" placeholder="请输入备注" v-model="formInfo.description" />
            </div>
          </a-col>
        </a-row>
        <a-row>
              ...
              <a-form-model-item label="" :label-col="{ span: 1 }" :wrapper-col="{ span: 22 }">
                <a-table :columns="areaTitle" :pagination="false" bordered :data-source="formInfo.areaArray">
                  ...
                  <span slot="areaPosiNums">
                    ...
                    区域车位数</span
                  >
                  <span slot="operationTitle" @click="handleTableAppend(index)" style="color: #0084ff; cursor: pointer">新增</span>
                  <template v-for="(com, ind) in areaTitle" :slot="com.dataIndex" slot-scope="item, record, index">
                    <div :key="com.dataIndex">
                      <a-form-model-item
                        v-if="['areaDescription'].includes(com.dataIndex)"
                        style="margin: -5px"
                        :label-col="{ span: 0 }"
                        :wrapper-col="{ span: 24 }"
                      >
                      ...
                      <a-form-model-item
                        v-else-if="['areaPosiNums'].includes(com.dataIndex)"
                      ...
                      <a-form-model-item
                        v-else-if="['areaZone'].includes(com.dataIndex)"
                      ...
                      <a-form-model-item
                        v-else-if="['areaName'].includes(com.dataIndex)"
                      ...
                      <a-form-model-item
                        v-else-if="['areaCode'].includes(com.dataIndex)"
                      ...
                      <div v-else-if="com.dataIndex === 'operation'">
                        <template v-if="formInfo.areaArray[index].parkingAreaId">
                          <span v-if="formInfo.areaArray[index].disabled" @click="handleTableEdit(index)" style="color: #0084ff; cursor: pointer"
                            >编辑&nbsp;&nbsp;</span
                          >
                          <span v-else @click="handleTableEditSave(index)" style="color: #0084ff; cursor: pointer">编辑保存&nbsp;&nbsp;</span>
                        </template>
                        <span
                          v-if="routeQuery.type === 'edit' && !formInfo.areaArray[index].parkingAreaId"
                          @click="handleTableSave(index)"
                          style="color: #0084ff; cursor: pointer"
                          >保存&nbsp;&nbsp;</span
                        >
                        <span @click="handleTableDelete(index)" style="color: red; cursor: pointer">删除</span>
                      </div>
                    </div>
                  </template>
                </a-table>
    ...
    <newSelectLngAndLat
      @cancel="cancel"
      :addressData="formInfo.placeAddr"
    ...
    >
      <div class="detail-item mt15">
        ...
        <div class="detail-item-content">{{ `${this.oneSearchValueData}（经纬度：${this.lngAndlatData}）` }}</div>
      </div>
    </newSelectLngAndLat>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    handleSave() {
      if (
        !Object.values(this.formInfo.location).every((item) => {
          return item;
        })
      ) {
    ...
    async afterParkApiDetail(value) {
      ...
      data.otherImg = (value.otherImg || '')
        .split(',')
        .filter((item) => item)
        .map((item) => ({ url: item, name: item.split('/').pop() }));
      data.carPlaceDoc = (value.carPlaceDoc || '')
        .split(',')
        .filter((item) => item)
        .map((item) => ({ url: item, name: item.split('/').pop() }));
