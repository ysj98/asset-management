...
<template>
  <div class="rent_view">
    ...
    <search-container size="fold" background="white" v-model="toggle">
      <div slot="headerBtns">
        <SG-Button icon="import" type="primary" :loading="exportBtnLoading" @click="handleExport" v-power="ASSET_MANAGEMENT.ASSET_RENT_VIEW_EXPORT"
          >导出</SG-Button
        >
      </div>
      <div slot="headerForm" style="margin-right: 32px; text-align: left">
        <a-row :gutter="8">
          ...
          <a-col :span="4">
            <a-select
              v-model="queryObj.contractStatusList"
              :options="$addTitle(contractStatusOptions)"
...
export default {
  ...
  methods: {
    ...
    queryTableData({ pageNum = 1, pageSize = 10, actionType }) {
      ...
      this.$api.assetRent
        ...
        .catch((message) => {
          ...
          this.$message.error(message || '查询列表数据错误');
        })
    ...
    handleExport() {
      ...
      exportDataAsExcel(this.queryTableData({ actionType: 'export' }), this.$api.assetRent.exportRentView, '资产出租一览表.xlsx', this).then(
